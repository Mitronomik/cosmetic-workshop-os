...
Статус: `IN PROGRESS` — A1a является первой focused delivery-частью этого runtime slice.

### Scope

PR #107 не реализует Slice A1. Slice A1 требует отдельного focused runtime PR; будущий PR number не назначен.
