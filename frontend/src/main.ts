...
function emptyRecipeLine(position = 1): RecipeLineForm { return { ingredient_id: '', position: String(position), phase: '', amount_value: '', amount_unit: 'percent', notes: '' }; }
...
function draftPercentTotal() { const total = recipesState.versionForm.ingredients.reduce((sum, line) => line.amount_unit === 'percent' && line.amount_value.trim() && /^-?\d+([.,]\d+)?$/.test(line.amount_value.trim()) ? sum + Number(line.amount_value.trim().replace(',', '.')) : sum, 0); return Number.isInteger(total) ? String(total) : String(Number(total.toFixed(4))); }
...
function loadRecipes(force = false) {
  ...
  Promise.all([getRecipeTemplates(), getIngredients(), getRecipeCatalogCategories(), getRecipeCatalogTags()])
    .then(([templates, ingredients, categories, tags]) => { recipesState.templates = templates.recipe_templates; recipesState.ingredients = ingredients.ingredients.filter((i)=>i.is_active); recipesState.catalogCategories = categories.categories; recipesState.catalogTags = tags.tags; recipesStatus = 'ready'; render(); })
