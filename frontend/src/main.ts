...
const helpArticles: HelpArticle[] = [
  { id: 'getting-started', title: 'Как начать работу', category: 'Первые шаги', summary: 'Безопасный порядок первого запуска: от checklist и демо до заказа и backup.', relatedSections: ['Главная', 'Демо-данные', 'Компоненты', 'Партии', 'Тара', 'Рецепты', 'Клиенты', 'Заказы', 'Резервные копии', 'Экспорт'], warning: 'Перед импортом, обновлением или большой правкой создайте backup или export.', body: ['Откройте главную страницу и пройдите onboarding checklist: он подсказывает, какие разделы заполнять первыми.', 'Если хотите потренироваться, сначала установите демо-данные. Они ставятся только явно и не нужны для реальной работы.', 'Добавьте компоненты, затем партии компонентов и движения прихода. Отдельно добавьте тару и её остатки.', 'Создайте рецепты и версии, затем клиентов и индивидуальные рецепты при необходимости.', 'Создайте заказ, проверьте готовность и запускайте производство только когда предупреждения понятны и материалов хватает.', 'Перед рискованными изменениями — импортом, обновлением приложения или массовыми правками — сделайте backup или export.'] },
  ...
  { id: 'backup-export', title: 'Backup и Export', category: 'Данные и безопасность', summary: 'Backup копирует SQLite-базу, export создаёт JSON-снимок данных.', relatedSections: ['Резервные копии', 'Экспорт'], warning: 'Создавайте backup перед импортом, обновлением приложения и большими изменениями.', body: ['Backup — это копия локальной SQLite-базы. Он нужен для восстановления рабочего состояния.', 'Export — JSON-снимок данных, удобный для проверки, переноса или будущих внешних сценариев.', 'Оба действия запускаются только явно пользователем. Приложение не создаёт их автоматически в этом PR.', 'Перед импортом, обновлением или массовыми правками сначала сделайте backup, а при необходимости ещё и export.'] },
  { id: 'import', title: 'Импорт', category: 'Импорт и демо-данные', summary: 'Импорт идёт через черновик, проверку, подтверждение и применение.', relatedSections: ['Импорт', 'Резервные копии'], warning: 'Проверьте ошибки и создайте backup до применения импорта.', body: ['Импорт поддерживает CSV и XLSX и сначала создаёт черновик с предпросмотром строк.', 'Пользователь выбирает цель, проверяет найденные ошибки и предупреждения, затем явно подтверждает применение.', 'Применение поддержано только для безопасных целей: компоненты, клиенты, рецепты и тара. Партии компонентов и заказы пока не применяются импортом.', 'Не применяйте файл, если значения выглядят не так: исправьте таблицу и загрузите заново.'] },
  { id: 'demo-data', title: 'Демо-данные', category: 'Импорт и демо-данные', summary: 'Демо помогает познакомиться с приложением без ручного заполнения.', relatedSections: ['Демо-данные', 'Главная'], body: ['Демо-данные необязательны и устанавливаются только по явному подтверждению.', 'Установка блокируется, если в базе уже есть реальные рабочие данные.', 'Демо-записи помечены «Демо ·», чтобы их не перепутать с настоящими.', 'Очистка удаляет только отслеженные демо-данные и тоже требует подтверждения. Демо не создаёт backup или export автоматически.'] },
];
...
const onboardingStepUi: Record<string, OnboardingStepUi> = {
  ...
  first_production: { hint: 'Проведите производство только после явного подтверждения; склад спишется после подтверждения.', sections: ['Заказы', 'Производство'] },
...
function render() {
  ...
  if (!root) return;
  const healthLabel = healthStatus === 'offline' ? 'Не удалось загрузить данные' : '';
  root.innerHTML = `
    <div class="app-shell">
      ...
      <main class="content">
        <header class="topbar">
          ...
          ${healthLabel ? `<span class="status ${healthStatus}">${healthLabel}</span>` : ``}
        </header>
...
function loadDashboard(force = false) {
  ...
  Promise.all([
    ...
    dashboardState = {
      status: 'ready',
      error: '',
      message: force ? 'Обзор обновлён. Данные перечитаны из локальной базы.' : '',
...
function dashboardReportsCard() { return `<section class="card data-card"><p class="card-kicker">Отчеты</p><h2>Сводка мастерской</h2><p>Посмотреть сводку по складу, заказам, производству и финансам.</p><div class="actions"><button class="secondary-action compact" type="button" data-nav-section="Отчеты">Открыть отчеты</button></div><p class="next-step">Кнопка только открывает read-only отчеты и не меняет данные.</p></section>`; }
...
function loadDemoDataStatus(force = false) {
  ...
  getDemoDataStatus()
    ...
    .catch(() => { demoDataUiState.status = 'error'; demoDataUiState.error = 'Не удалось загрузить данные. Перезапустите «Мастерскую косметолога» и повторите попытку.'; render(); });
}
...
function demoLoadErrorCard() { return `<section class="card error-card"><h2>Статус недоступен</h2><p>Не удалось загрузить данные. Перезапустите «Мастерскую косметолога» и повторите попытку.</p><div class="actions"><button class="secondary-action" type="button" data-action="reload-demo-data">Повторить</button></div></section>`; }
...
function demoBoundariesCard() { return `<section class="card data-card"><p class="card-kicker">Без скрытых действий</p><h2>Честные границы демо-режима</h2><ul class="checklist compact-list"><li>Демо-данные не устанавливаются автоматически</li><li>Демо-режим не удаляет реальные данные</li><li>Backup/export не создаются автоматически</li><li>Производство не запускается автоматически</li><li>Партии и заказы не применяются через импорт</li><li>Облачная синхронизация, OCR и PDF не относятся к демо-режиму</li></ul></section>`; }
function demoCountLabel(key: string): string { return ({ ingredients: 'Компоненты', ingredient_lots: 'Партии компонентов', stock_movements: 'Складские движения', packaging_items: 'Тара', packaging_stock_movements: 'Движения тары', recipe_templates: 'Рецепты', recipes: 'Рецепты', recipe_versions: 'Версии рецептов', recipe_ingredients: 'Составы рецептов', clients: 'Клиенты', client_recipes: 'Индивидуальные рецепты', orders: 'Заказы', alerts: 'Алерты', production_runs: 'Производство', production_batches: 'Производство', purchase_suggestions: 'Закупочный список' } as Record<string, string>)[key] ?? 'Другие данные'; }
...
function loadImports(force = false) {
  ...
  Promise.all([getImportTargets(), getImportDrafts(importUiState.filters)])
    ...
    .catch(() => {
      ...
      importUiState.error = 'Не удалось загрузить данные. Перезапустите «Мастерскую косметолога» и повторите попытку.';
      render();
    });
}
...
function importApplyErrorMessage(error: unknown) {
  ...
  if (status === 409) return `Применение остановлено: есть конфликт или дубликат. Исправьте файл и создайте новый черновик.${details}`;
...
function backupPage() {
  ...
  return `<div class="page-grid backup-page">
    <section class="card data-card dashboard-hero">
      <div><p class="card-kicker">Локальная защита данных</p><h2>Резервные копии</h2><p>Создавайте локальную копию базы перед импортом, обновлением приложения или крупными изменениями данных.</p><p class="next-step">Резервная копия не меняет рецепты, клиентов, заказы, склад и производство. Это отдельный файл SQLite в локальной папке backups.</p></div>
...
function backupStatusCards() {
  ...
  return `<section class="overview-grid">
    ...
    <div class="metric-card wide"><span>Расположение базы</span><strong class="path-text">${escapeHtml(status.database_path)}</strong></div>
    <div class="metric-card"><span>Размер базы</span><strong>${formatFileSize(status.database_size_bytes)}</strong></div>
    <div class="metric-card wide"><span>Папка резервных копий</span><strong>${status.backup_dir_exists ? 'Папка backups найдена' : 'Папка backups ещё не создана. Она появится после первой явной резервной копии.'}</strong><small class="path-text">${escapeHtml(status.backup_dir)}</small></div>
...
function backupHistoryItem(backup: BackupFileResponse) { return `<article class="recipe-line backup-item"><div class="section-heading"><div><h3>${escapeHtml(backup.filename)}</h3><p><span class="pill info">${backupReasonLabel(backup.reason)}</span> <span class="pill muted">${formatFileSize(backup.size_bytes)}</span></p></div><small>${formatDateTime(backup.created_at || '')}</small></div><p><strong>Локальный путь:</strong><br><code class="path-text">${escapeHtml(backup.path)}</code></p><p class="next-step">Только файл копии. Восстановление сейчас недоступно.</p></article>`; }
function backupNonGoalsCard() { return `<section class="card data-card"><p class="card-kicker">Честные границы</p><h2>Текущие границы</h2><p>Сейчас этот экран создаёт и показывает локальные резервные копии. Восстановление из копии, скачивание через браузер, автоматические копии и импорт из копии сейчас недоступны.</p><ul class="checklist compact-list"><li>Восстановление из копии</li><li>Скачивание копии из браузера</li><li>Автоматические копии по расписанию</li><li>Облачные резервные копии</li><li>Export CSV/XLSX</li><li>Import CSV/XLSX</li></ul></section>`; }
...
function ingredientsPage() {
  ...
  if (ingredientsStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Компоненты</p><h2>Не удалось загрузить компоненты</h2><p>${ingredientsError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-ingredients">Повторить загрузку</button></section>`;
...
function packagingItemsPage() {
  ...
  if (packagingItemsStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Тара</p><h2>Не удалось загрузить тару</h2><p>${packagingItemsError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-packaging-items">Повторить загрузку</button></section>`;
...
function ingredientLotsPage() {
  ...
  if (ingredientLotsStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Приходы и партии</p><h2>Не удалось загрузить приходы и партии</h2><p>${ingredientLotsError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-ingredient-lots">Повторить загрузку</button></section>`;
...
function stockMovementsPage() {
  ...
  if (stockMovementsStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Движения сырья</p><h2>Не удалось загрузить движения сырья</h2><p>${stockMovementsError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-stock-movements">Повторить загрузку</button></section>`;
...
function stockBalanceCard() {
  if (!stockMovementsState.selectedLotId) return `<p class="next-step">Выберите партию, чтобы увидеть текущий остаток и историю движений.</p>`;
...
function inventoryPage() {
  ...
  if (inventoryStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Склад</p><h2>Не удалось загрузить склад</h2><p>${inventoryError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-inventory">Повторить загрузку</button></section>`;
  const overview = inventoryState.overview;
  return `<div class="inventory-layout"><section class="card inventory-intro"><p class="card-kicker">Склад</p><h2>Обзор запасов</h2><p>Здесь показаны текущие остатки, рассчитанные по сохранённой истории движений. На этой странице нет действий изменения склада.</p></section>${overview ? overviewCards(overview) : emptyCard('Сводка пока пуста', 'Когда появятся партии компонентов или тара, здесь будет краткая сводка.')} ${ingredientLotsTable(inventoryState.ingredientLots)} ${packagingTable(inventoryState.packagingItems)}</div>`;
}
...
function reportDocumentsBoundariesCard() { return `<section class="card data-card"><p class="card-kicker">Честные границы</p><h2>Что важно знать</h2><ul class="checklist compact-list"><li>Markdown остается доступен; PDF создается только явным действием, если на этом устройстве доступна поддержка.</li><li>DOCX сейчас не создается.</li><li>Документ строится из сохранённых отчетов и не пересчитывает данные на странице.</li><li>Создание документа не меняет заказы, склад, производство или закупки.</li><li>Это не бухгалтерский и не налоговый документ.</li><li>Налоговые ставки не придумываются.</li></ul></section>`; }
...
function helpFilters() {
  return `<section class="card filter-card"><div class="catalog-toolbar"><label>Поиск<input data-action="filter-help-search" type="search" value="${escapeHtml(helpUiState.search)}" placeholder="Найти: рецепт, партия, импорт, backup…" /></label><label>Категория<select data-action="filter-help-category"><option value="">Все категории</option>${helpCategories.map((category) => `<option value="${escapeHtml(category)}" ${helpUiState.category === category ? 'selected' : ''}>${escapeHtml(category)}</option>`).join('')}</select></label></div><div class="actions"><button class="secondary-action compact" type="button" data-action="reset-help-filters">Сбросить фильтр</button></div></section>`;
}
function helpArticleList(articles: HelpArticle[]) {
  if (articles.length === 0) return `<section class="card empty-card"><h2>Подсказки не найдены</h2><p>По этому запросу подсказок нет. Попробуйте другое слово: рецепт, склад, импорт, backup.</p><button class="secondary-action" type="button" data-action="reset-help-filters">Сбросить фильтр</button></section>`;
...
function plannedSectionPlaceholder(section: NavigationSection) {
  const copy: Record<string, { title: string; now: string }> = {
    ...
    Отчеты: { title: 'Отчеты', now: 'Сводные отчеты доступны в разделе /reports. Если вы видите этот экран, вернитесь через пункт «Отчеты» в навигации.' },
...
function recipesPage() {
  if (recipesStatus === 'idle' || recipesStatus === 'loading') return `<section class="card"><p class="card-kicker">Рецепты</p><h2>Загружаем рецепты…</h2><p>Загружаем рецепты, версии и компоненты.</p></section>`;
  if (recipesStatus === 'error') return `<section class="card error-card"><p class="card-kicker">Рецепты</p><h2>Не удалось загрузить рецепты</h2><p>${recipesError || 'Не удалось загрузить данные.'}</p><p class="next-step">Проверьте, что приложение запущено полностью, и попробуйте обновить раздел.</p><button class="primary-action" type="button" data-action="reload-recipes">Повторить загрузку</button></section>`;
...
function draftPercentHint() { const total = draftPercentTotalNumber(); const isExact = Math.abs(total - 100) < 0.000001; return `<p class="next-step ${isExact ? 'success-text' : 'warning-text'}"><strong>Сумма процентов:</strong> ${escapeHtml(draftPercentTotal())}%. ${isExact ? 'Сумма процентов ровно 100%. После сохранения будет выполнен итоговый расчет версии.' : 'Проверьте сумму процентов. Итоговый расчет выполняется после сохранения версии.'}</p>`; }
...
function loadStockMovements(force = false) {
  ...
  }).catch(() => { stockMovementsStatus = 'error'; stockMovementsError = 'Не получилось загрузить партии компонентов.'; render(); });
}
...
function loadRecipes(force = false) {
  ...
  Promise.all([getRecipeTemplates(), getIngredients(), getRecipeCatalogCategories(), getRecipeCatalogTags()])
    ...
    .catch(() => { recipesStatus = 'error'; recipesError = 'Не получилось загрузить рецепты. Проверьте локальное приложение и попробуйте обновить раздел.'; render(); });
}
...
function loadIngredientLots(force = false) {
  ...
  Promise.all([getIngredientLots(), getIngredients()])
    ...
    .catch(() => { ingredientLotsStatus = 'error'; ingredientLotsError = 'Не получилось загрузить партии компонентов.'; render(); });
}
...
function createReportDocumentFromUi(format: 'markdown' | 'pdf' = 'markdown') {
  ...
  if (!reportDocumentsUiState.documentStatus?.can_create) { reportDocumentsUiState.error = 'Сейчас нельзя создать документ отчета.'; reportDocumentsUiState.message = ''; render(); return; }
...
function loadReports(force = false) {
  ...
  Promise.all([getReportsOverview(), getInventoryReport(), getOrdersReport(), getProductionReport(), getFinanceReport()])
    .then(([overview, inventory, orders, production, finance]) => { reportsUiState = { ...reportsUiState, status: 'ready', error: '', message: force ? 'Отчеты обновлены. Данные обновлены из локальной базы.' : '', overview, inventory, orders, production, finance }; render(); })
    .catch((error: unknown) => { reportsUiState.status = 'error'; const message = error instanceof Error && error.message && error.message !== 'API request failed' ? error.message : 'Не удалось загрузить отчеты. Проверьте, что локальное приложение запущено, и попробуйте снова.'; reportsUiState.error = message; reportsUiState.message = ''; render(); });
}
