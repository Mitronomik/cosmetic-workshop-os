/**
 * The CR-009 B3 manual-backup audit contract, as the frontend is allowed to see it.
 *
 * Durable contract: `docs/decisions/0013-file-backed-artifact-audit-semantics.md`,
 * `docs/decisions/0015-sqlite-backup-consistency-and-manual-audit.md` and
 * `docs/backup-and-restore.md`.
 *
 * Two results arrive in one HTTP 201 response and must stay visibly separate:
 * the backup file was created (primary, authoritative) and the Journal entry was
 * or was not recorded (secondary). A pending Journal entry is **not** a failed
 * backup, so it must never be rendered as an error and must never provoke a
 ...
 *
 * This deliberately mirrors `export-audit-contract.ts` and
 * `report-document-audit-contract.ts` rather than generalizing them. The three
 * artifact kinds have different backend-owned Russian wording, and a shared
 * "artifact" abstraction would make it possible to show an export's warning on
 * the backups page.
 */

/** The exact accepted warning for one backup whose Journal entry is pending. */
export const BACKUP_PENDING_AUDIT_MESSAGE =
  'Резервная копия создана, но запись в журнал действий пока не добавлена. Приложение повторит попытку при следующем запуске или перед созданием следующей резервной копии.';

/** The exact accepted warning while any backup still awaits its Journal entry. */
export const BACKUP_PENDING_AUDIT_COUNT_WARNING =
  'Некоторые созданные резервные копии ещё не добавлены в журнал действий. Приложение повторит попытку при следующем запуске или перед созданием следующей резервной копии.';

export type BackupAuditStatus = 'recorded' | 'pending';

export type BackupAuditResult = {
  /** False when the 201 body does not carry a complete, recognized audit contract. */
  valid: boolean;
  status: BackupAuditStatus | null;
  /** The exact backend warning for `pending`, and empty for `recorded`. */
  warning: string;
};

const invalid: BackupAuditResult = { valid: false, status: null, warning: '' };
 ...
 * success: the backup may well exist, so the caller must fall back to its
 ...
 */
export function backupAuditResult(response: unknown): BackupAuditResult {
  ...
  if (status === 'pending') {
    return message === BACKUP_PENDING_AUDIT_MESSAGE
      ? { valid: true, status: 'pending', warning: BACKUP_PENDING_AUDIT_MESSAGE }
      : invalid;
  }
  return invalid;
}
 ...
 * no backup is awaiting a Journal entry, and the UI clears a standing warning on
 ...
 */
export function backupPendingAuditCount(status: unknown): number | null {
 ...
 *
 * One region, not two. The just-created backup's warning takes precedence while
 ...
 */
export function backupAuditWarning(state: { auditWarning: string; pendingAuditCount: number | null }): string {
  if (state.auditWarning) return state.auditWarning;
  return (state.pendingAuditCount ?? 0) > 0 ? BACKUP_PENDING_AUDIT_COUNT_WARNING : '';
}
 ...
 * confirms nothing.
 */
export function adoptBackupPendingAuditCount(state: BackupAuditUiState, status: unknown): void {
  const count = backupPendingAuditCount(status);
  if (count === null) return;
  state.pendingAuditCount = count;
  if (count === 0) state.auditWarning = '';
}

export type BackupAuditUiState = { auditWarning: string; pendingAuditCount: number | null };
