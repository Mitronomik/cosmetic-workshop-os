...
function packagingCatalogPanel() {
  ...
  if (!item) return `<section class="card catalog-helper-card"><p class="card-kicker">Организация каталога тары</p><h2>Моя группа и метки</h2><p>${helperCopy}</p><p>Группы и метки созданы. Чтобы назначить их конкретной таре, нажмите «Изменить» у нужной тары.</p>${availableCatalog}${createControls}<p class="next-step">Созданные группы и метки появляются здесь сразу. Они не добавляются в выпадающий список «Тип тары».</p></section>`;
...
function packagingAvailableCatalogLists() {
  const categories = packagingItemsState.catalogCategories.length === 0 ? '<p class="empty-hint">Группы пока не созданы.</p>' : `<div class="tag-picker">${packagingItemsState.catalogCategories.map((category) => `<span class="tag-chip selected">${escapeHtml(category.name)}</span>`).join('')}</div>`;
