...
import { bindEveryActionControl, LocalArtifactsReportsFeedbackLifecycle, type LocalArtifactRouteKey } from './local-artifacts-reports-feedback.js';
import { createLocalArtifactRouteRuntime } from './local-artifacts-reports-runtime.js';
import { localArtifactReportPresentation } from './local-artifacts-reports-presentation.js';
...
function bindEvents(root: HTMLElement) {
  ...
  bindEveryActionControl(root, '[data-action="reload-reports" data-focus-key="b3-reports-refresh"]', () => loadReports(true));
  bindEveryActionControl(root, '[data-action="reload-report-documents" data-focus-key="b3-report-documents-refresh"]', () => loadReportDocuments(true));
  root.querySelector<HTMLFormElement>('[data-form="report-document-create" data-focus-key="b3-report-documents-create"]')?.addEventListener('submit', submitReportDocumentCreateForm);
  ...
  bindEveryActionControl(root, '[data-action="reload-backups" data-focus-key="b3-backups-refresh"]', () => loadBackups(true));
  bindEveryActionControl(root, '[data-action="reload-exports" data-focus-key="b3-exports-refresh"]', () => loadExports(true));
  ...
  root.querySelector<HTMLFormElement>('[data-form="export-create" data-focus-key="b3-exports-create"]')?.addEventListener('submit', submitExportCreateForm);
  ...
  root.querySelector<HTMLFormElement>('[data-form="backup-create" data-focus-key="b3-backups-create"]')?.addEventListener('submit', submitBackupCreateForm);
...
function refreshBackupsAfterMutation() { backupRuntime.load('mutation-refresh'); }
...
function refreshExportsAfterMutation() { exportRuntime.load('mutation-refresh'); }
...
function exportPage() {
  ...
  return `<div class="page-grid backup-page">
    ...
    ${exportUiState.warning ? feedbackMessage('warning', exportUiState.warning, '<p class="next-step"><button class="secondary-action compact" type="button" data-action="reload-exports" data-focus-key="b3-exports-refresh" data-focus-key="b3-exports-refresh">Обновить список</button></p>') : ''}
...
function applyBackupLifecycleState() {
  localArtifactReportPresentation('backups', backupLifecycle, Boolean(backupUiState.backupStatus?.database_exists));
...
function handleLifecycleFinish(result: { accepted: boolean; announcement: 'none' | 'polite' | 'assertive'; message: string; focusKey?: string | null }) { if (!result.accepted) return; if (result.message) { if (result.announcement === 'assertive') announceAssertive(result.message); else if (result.announcement === 'polite') announcePolite(result.message); } focusLocalArtifactTarget(result.focusKey ?? null); }
...
function backupPage() {
  ...
  return `<div class="page-grid backup-page">
    ...
    ${backupUiState.warning ? feedbackMessage('warning', backupUiState.warning, '<p class="next-step"><button class="secondary-action compact" type="button" data-action="reload-backups" data-focus-key="b3-backups-refresh" data-focus-key="b3-backups-refresh">Обновить список</button></p>') : ''}
...
function reportsErrorCard() { return `<section class="card error-card"><p class="card-kicker">Отчёты</p><h2>Не удалось загрузить отчёты</h2><p>${escapeHtml(reportsUiState.error || 'Не удалось загрузить отчёты. Проверьте, что приложение запущено, и попробуйте снова.')}</p><button class="primary-action" type="button" data-action="reload-reports" data-focus-key="b3-reports-refresh">Повторить</button></section>`; }
...
function refreshReportDocumentsAfterMutation() { reportDocumentsRuntime.load('mutation-refresh'); }
