...
**Current merged baseline.** `origin/main` is `87410910aad472343c057f0bcbfcc3797f8b8e09` (the PR #157 merge commit), and the current accepted merged backend result is `942 passed / 0 failed / 0 skipped`, up from `883` because PR #157 added report-financial regression coverage with all `883` baseline node IDs still collected. The earlier values `7369e7f133f0ce02aea5f2021cbb0e14104b7b34` (PR #156), `d432fcaee52a16a4f8b609ec160cf3fa2b33d013` (PR #154) and `c3a3a7b8db06fe85290216113b784123ed9b6b30` (PR #152) are **historical**.
...
**`C3-I — Read-only AuditLog workspace` is implemented on its PR branch and not merged.** It is the only authorized C3 runtime slice: `IMPLEMENTED ON PR BRANCH — NOT MERGED`. It adds one read-only endpoint `GET /api/audit-logs` and one screen `/settings/audit-log` titled `Журнал действий`, reachable from `Данные и настройки`, so the user can read a plain-language history of important workshop actions without opening SQLite, JSON, logs, GitHub or a terminal. The screen offers a date range, action, entity and actor filters, `Очистить фильтры`, `Обновить` and `Показать ещё`, and it never shows raw codes, the raw stored summary, metadata or internal IDs. It adds **no migration** and changes no AuditLog write behavior. Backup, export, report-document and workshop-profile actions are still not audited, so the journal does not show them; closing that gap needs a separately authorized write slice.
...
The product is **not** release-ready. The remaining release obligations are: the `CR-004` SQLite backup transaction-consistency investigation; the Restore product decision and implementation; final macOS packaging and user-ready launch; installation verification; the packaged update flow and update smoke; the full release-candidate smoke; the `C3-I` read-only AuditLog workspace, which is **implemented on its PR branch and awaiting review and merge**; C4, which remains **inactive** and still needs a product decision; and continuing documentation accuracy. The C1 tax-setting implementation and the whole C2 financial implementation are no longer outstanding obligations: C1 and C2 are both merged and **COMPLETED**.
...
The current focus is the **bounded `C3-I` read-only AuditLog workspace**. `R3`, `R2`, and `R4` are all merged and DONE, the backend baseline correction gate is **DONE**, `CR-007` is accepted, and `C1-I` is merged and **DONE**. `CR-008` records the accepted C2 calculation and immutable-snapshot contract; `C2-I` (PR #151), `C2-II` (PR #152), `C2-III-A` (PR #154) and `C2-III-B` (PR #157) are all merged and `DONE`, and `main` is now `fa433d03acbf68e16b14ba6245885ab9eaf15c35`. **One runtime implementation slice is open: `C3-I`, implemented on its PR branch and not merged.** The former `C2-III` umbrella was subdivided into exactly `C2-III-A` and `C2-III-B`, and both are merged. Reports on merged `main` are snapshot-backed. **C2 is COMPLETED.** The only authorized runtime slice is `C3-I`, which is implemented on the branch `codex/c3-i-read-only-audit-log-workspace` and is not merged; its durable contract is `docs/audit-log.md`.
...
`R4 — Canonical backup/export filename reason normalization` delivered that contract as one bounded slice covering both nodes through a single shared backend helper, and it is **merged and DONE** as PR #146. `CR-006` is a `needs evidence` row and is **not active**. `CR-004` remains separate and inactive. C1 has an accepted product contract (`CR-007`) and a merged implementation (`C1-I`, PR #149), so C1 is **DONE**. C2 has an accepted product contract (`CR-008`) and a **complete implementation**: `C2-I`, `C2-II`, `C2-III-A` and `C2-III-B` are all merged and exact-head verified, so **C2 is COMPLETED**. C3 has one authorized runtime slice, `C3-I` (`docs/audit-log.md`), implemented on its PR branch and not merged; C4 remains inactive. Restore remains open, packaging/install/update work and the full release-candidate smoke remain open, and **product release readiness is not claimed**. Contract: `state/current-focus.md` and `docs/implementation-plan.md`. Evidence: `docs/backend-baseline-failure-triage.md`.
