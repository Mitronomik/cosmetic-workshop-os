...
function clientRecipePageMutationActive() { return clientRecipeCreateSubmitting || clientRecipeCompositionSubmitting; }
...
function selectClientRecipeTemplate(value: string) { if (clientRecipePageMutationActive()) return; saveClientRecipeFormFromDom(); clientRecipesState.form.recipe_template_id = value; clientRecipesState.form.source_recipe_version_id = ''; clientRecipesState.selectedTemplateId = value ? Number(value) : null; clientRecipesState.versions = []; clientRecipesState.versionsStatus = value ? 'loading' : 'idle'; clientRecipesError = ''; if (!value) { render(); return; } render(); getRecipeVersions(Number(value)).then((response) => { if (clientRecipePageMutationActive()) return; saveClientRecipeFormFromDom(); clientRecipesState.versions = response.recipe_versions; clientRecipesState.versionsStatus = 'ready'; render(); }).catch(() => { if (clientRecipePageMutationActive()) return; clientRecipesState.versionsStatus = 'error'; clientRecipesError = 'Не удалось загрузить сохранённые версии состава. Попробуйте выбрать базовый рецепт ещё раз.'; render(); }); }
...
function openClientRecipeCompositionEditor() { if (clientRecipePageMutationActive()) return; const detail = clientRecipesState.selectedDetail; if (!detail || !detail.client_recipe.is_active || detail.client_recipe.status === 'archived') return; const open = () => { clientRecipesState.compositionEditor = { isOpen: true, isSaving: false, draft: clientRecipeCompositionDraftFromDetail(detail), error: '' }; clientRecipeCompositionValidation = emptyFormValidationState(); render(); }; if (recipesState.ingredients.length === 0) getIngredients().then((r)=>{ recipesState.ingredients = r.ingredients; open(); }).catch(()=>{ clientRecipesState.compositionEditor.error = 'Не удалось обновить список компонентов. Попробуйте ещё раз.'; render(); }); else open(); }
...
function clearFieldValidationDom(prefix: 'client' | 'ingredient' | 'ingredient-lot' | 'stock-movement' | 'packaging-item' | 'recipe-template' | 'recipe-version' | 'client-recipe' | 'client-recipe-composition', field: string, control?: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
