...
import { canOpenOrderProductionConfirmation, canStartOrderReadinessRequest, clearOrderSourceValidation, createOrderMutationController, emptyOrderValidationProvenance, orderOperationError, orderOperationErrorFor, orderPayloadFromDraft, orderProductionIsClosed, orderReadinessRequestActive, orderReadinessResultIsCurrent, orderRequestOwnerMatches, ownerFromOrderRequest, productionReadinessFailureMessage, type OrderContextSnapshot, type OrderOperationError, type OrderRequestSnapshot, type OrderTransientRequestOwner, type OrderValidationProvenance } from './order-mutation-lifecycle.js';
...
let ordersState: OrdersState = { items: [], clients: [], templates: [], versions: [], clientRecipes: [], packagingItems: [], formMode: 'create', form: emptyOrderForm(), showForm: false, selectedOrder: null, includeInactive: true, filters: { search: '', status: 'active' }, referenceLoading: false, referenceError: '', readinessByOrderId: {}, readinessAttemptGenerationByOrderId: {}, readinessResultGenerationByOrderId: {}, readinessCheckedOrderUpdatedAtByOrderId: {}, readinessLoadingOrderId: null, readinessRequestOwner: null, readinessError: '', readinessErrorInfo: null, productionByOrderId: {}, productionLoadingOrderId: null, productionRequestOwner: null, productionHistoryRequestOwner: null, productionError: '', productionErrorInfo: null, productionConfirmingOrderId: null, productionNotesByOrderId: {} };
...
function currentOrderReadiness(o: Order) { const readiness=ordersState.readinessByOrderId[o.id]; return orderReadinessResultIsCurrent(o, readiness, ordersState.readinessAttemptGenerationByOrderId[o.id], ordersState.readinessResultGenerationByOrderId[o.id], ordersState.readinessCheckedOrderUpdatedAtByOrderId[o.id]) ? readiness : null; }
function orderLifecycleButtons(o: Order) { if (!o.is_active || o.status==='archived') return ''; const busy=orderReadinessBusy(o.id); if (o.status==='cancelled') return `<button class="secondary-action compact danger-action" type="button" data-action="archive-order" data-id="${o.id}" ${busy?'disabled':''}>Архивировать</button>`; return `<button class="secondary-action compact danger-action" type="button" data-action="cancel-order" data-id="${o.id}" ${busy?'disabled':''}>Отменить заказ</button><button class="secondary-action compact danger-action" type="button" data-action="archive-order" data-id="${o.id}" ${busy?'disabled':''}>Архивировать</button>`; }
...
function orderDetailPanel(o: Order) {
  ...
  const readinessAction = closed
    ...
    : `<button class="primary-action" type="button" data-action="check-order-readiness" data-id="${o.id}" ${readinessBusy ? 'disabled aria-busy="true"' : ''}>${readinessBusy ? 'Проверяем…' : 'Проверить изготовление'}</button>`;
  return `<section class="card data-card"><div class="section-heading"><div><p class="card-kicker">Карточка заказа</p><h2>${escapeHtml(o.product_name)}</h2></div><button class="secondary-action" type="button" data-action="close-order-detail">Закрыть карточку</button></div><p><strong>Клиент:</strong> ${escapeHtml(orderClientName(o.client_id))}</p><p><strong>Основа:</strong> ${orderSourceLabel(o)}</p><p><strong>Партия:</strong> ${quantityLabel(o.target_batch_size_value, o.target_batch_size_unit)}</p><p><strong>Тара:</strong> ${orderPackagingLabel(o)}</p><p><strong>Статус:</strong> ${orderStatusLabel(o.status)}</p><p><strong>Дата заказа:</strong> ${o.ordered_at?formatDate(o.ordered_at):'Не указана'} · <strong>План производства:</strong> ${o.planned_production_at?formatDate(o.planned_production_at):'Не указан'}</p><p><strong>Цена:</strong> ${o.sale_price?`${escapeHtml(o.sale_price)} ₽`:'Не указана'}</p><p><strong>Заметки:</strong><br>${escapeHtml(o.notes || 'Нет заметок')}</p>${o.status==='cancelled'?'<p class="next-step">Отменённый заказ нельзя редактировать.</p>':(!o.is_active||o.status==='archived')?'<p class="next-step">Архивный заказ нельзя редактировать.</p>':readinessBusy?'<p class="next-step">Дождитесь завершения проверки перед изменением, отменой или архивированием этого заказа.</p>':'<p class="next-step">Можно изменить только безопасные поля заказа. Производственные статусы здесь не меняются.</p>'}<div class="actions">${(o.status==='cancelled'||!o.is_active||o.status==='archived')?'':`<button class="secondary-action" type="button" data-action="edit-order" data-id="${o.id}" ${readinessBusy?'disabled':''}>Изменить заказ</button>`}${readinessAction}${orderLifecycleButtons(o)}</div></section>${orderReadinessPanel(o)}${orderProductionPanel(o)}`;
}
function orderReadinessPanel(o: Order) {
  ...
  if (orderReadinessBusy(o.id)) return `<section class="card data-card" data-order-readiness-busy="true" aria-busy="true" aria-labelledby="order-readiness-loading-title"><p class="card-kicker">Проверка изготовления</p><h2 id="order-readiness-loading-title">Проверяем…</h2><p>Сверяем рецепт, компоненты, партии и тару.</p><p class="next-step">Это только проверка. Склад не списан, партии не зарезервированы, заказ не переведён в производство.</p></section>`;
  const readinessError = orderOperationErrorFor(ordersState.readinessErrorInfo, o.id);
  if (readinessError) return `<section class="card error-card" data-order-readiness-error="true"><p class="card-kicker">Сбой проверки</p><h2>Результат готовности не получен</h2><p>${escapeHtml(readinessError)}</p><p class="next-step">Это системная ошибка запроса, а не результат «производство заблокировано». Можно безопасно повторить только проверку.</p><button class="primary-action" type="button" data-action="retry-order-readiness" data-id="${o.id}">Повторить проверку</button></section>`;
...
function orderProductionPanel(o: Order) {
  ...
  if (orderProductionClosed(o)) { const canOpen=o.status==='produced'||o.status==='delivered'; return `<section class="card data-card"><p class="card-kicker">Изготовление</p><h2>Изготовление недоступно</h2><p class="next-step">Для закрытых, выданных, архивных или уже изготовленных заказов действие изготовления не показывается.</p>${orderOperationErrorFor(ordersState.productionErrorInfo, o.id) ? `<p class="page-message error-message">${escapeHtml(orderOperationErrorFor(ordersState.productionErrorInfo, o.id))}</p>` : ''}${canOpen ? `<div class="actions"><button class="secondary-action" type="button" data-action="open-order-production-batch" data-id="${o.id}" ${ordersState.productionLoadingOrderId===o.id?'disabled':''}>${ordersState.productionLoadingOrderId===o.id?'Открываем…':'Открыть партию'}</button></div>` : ''}</section>`; }
  const readiness = currentOrderReadiness(o);
  if (!readiness) return `<section class="card data-card"><p class="card-kicker">Изготовление</p><h2>Подтверждение изготовления</h2><p class="next-step">${ordersState.readinessByOrderId[o.id]?'Текущий результат проверки отсутствует. Повторите проверку перед изготовлением.':'Сначала проверьте готовность изготовления.'}</p></section>`;
...
function readinessIssueContext(issue: ProductionReadinessIssue, result: ProductionReadinessResponse) {
  ...
  if (issue.entity_type === 'ingredient_lot') { const lot=result.ingredients.flatMap((line)=>line.selected_lots).find((item)=>item.lot_id===issue.entity_id); return lot?.lot_code ? `Партия «${lot.lot_code}»` : 'Партия компонента'; }
...
function readinessIssuesSection(title: string, issues: ProductionReadinessIssue[], empty: string, result: ProductionReadinessResponse) { const groups=new Map<string,ProductionReadinessIssue[]>(); issues.forEach((issue)=>{ const context=readinessIssueContext(issue,result); groups.set(context,[...(groups.get(context)||[]),issue]); }); return `<div class="readiness-block"><h3>${title}</h3>${issues.length ? [...groups.entries()].map(([context,items])=>`<div class="readiness-issue-group"><h4>${escapeHtml(context)}</h4><ul>${items.map((issue) => `<li><span class="pill ${issue.severity === 'blocking' ? 'danger' : issue.severity === 'warning' ? 'warning' : 'info'}">${readinessSeverityLabel(issue.severity)}</span> ${escapeHtml(issue.message)}</li>`).join('')}</ul></div>`).join('') : `<p class="empty-hint">${empty}</p>`}</div>`; }
...
function resetInvalidatedOrderOperationState(){ orderMutationController.invalidateRequest('readiness'); orderMutationController.invalidateRequest('production'); orderMutationController.invalidateRequest('productionHistory'); ordersState.readinessRequestOwner=null; ordersState.productionRequestOwner=null; ordersState.productionHistoryRequestOwner=null; ordersState.readinessLoadingOrderId=null; ordersState.productionLoadingOrderId=null; ordersState.productionConfirmingOrderId=null; ordersState.readinessError=''; ordersState.productionError=''; ordersState.readinessErrorInfo=null; ordersState.productionErrorInfo=null; }
...
function checkOrderReadiness(id:number){ const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; if(!canStartOrderReadinessRequest(orderFormMutationActive(), order, ordersState.readinessRequestOwner, ordersState.readinessLoadingOrderId, id)) return; const snapshot=orderMutationController.beginRequest('readiness', currentOrderContext(), { requestedOrderId: id }); ordersState.readinessAttemptGenerationByOrderId[id]=snapshot.generation; ordersState.readinessRequestOwner=ownerFromOrderRequest(snapshot, id, 'readiness'); ordersState.readinessLoadingOrderId=id; ordersState.readinessError=''; ordersState.readinessErrorInfo=null; ordersState.productionConfirmingOrderId=null; announcePolite('Проверяем готовность заказа к изготовлению.'); render(); checkOrderProductionReadiness(id).then((result)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !readinessOwnerMatches(snapshot, id)) return; ordersState.readinessLoadingOrderId=null; ordersState.readinessRequestOwner=null; if(result.order_id!==id){ ordersState.readinessError='Локальное приложение вернуло результат для другого заказа. Данные не применены; повторите проверку.'; ordersState.readinessErrorInfo=orderOperationError(id,ordersState.readinessError); announceAssertive(ordersState.readinessError); render(); return; } const currentOrder=ordersState.selectedOrder; ordersState.readinessByOrderId[id]=result; ordersState.readinessResultGenerationByOrderId[id]=snapshot.generation; ordersState.readinessCheckedOrderUpdatedAtByOrderId[id]=currentOrder.updated_at; ordersState.readinessError=''; ordersState.readinessErrorInfo=null; announcePolite(result.can_produce?'Проверка завершена. Заказ готов к изготовлению.':'Проверка завершена. Есть замечания, которые нужно просмотреть.'); render(); }).catch((e)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !readinessOwnerMatches(snapshot, id)) return; ordersState.readinessLoadingOrderId=null; ordersState.readinessRequestOwner=null; const status=typeof e==='object'&&e&&'status' in e?Number((e as {status?:number}).status):undefined; const message=e instanceof Error?e.message:''; ordersState.readinessError=productionReadinessFailureMessage({status,message,networkFailure:e instanceof TypeError}); ordersState.readinessErrorInfo=orderOperationError(id, ordersState.readinessError); announceAssertive(ordersState.readinessError); render(); }); }
function openProductionConfirmation(id:number){ const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=order ? currentOrderReadiness(order) : null; if(!canOpenOrderProductionConfirmation(orderFormMutationActive(), order, readiness)) return; ordersState.productionConfirmingOrderId=id; ordersState.productionError=''; ordersState.productionErrorInfo=null; if(!(id in ordersState.productionNotesByOrderId)) ordersState.productionNotesByOrderId[id]=''; render(); }
function cancelProductionConfirmation(id:number){ if (orderFormMutationActive()) return; if(ordersState.productionLoadingOrderId===id) return; ordersState.productionConfirmingOrderId=null; ordersState.productionError=''; ordersState.productionErrorInfo=null; render(); }
function confirmProduction(id:number){ if (orderFormMutationActive()) return; const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=order ? currentOrderReadiness(order) : null; if(!canOpenOrderProductionConfirmation(false, order, readiness)) return; const snapshot=orderMutationController.beginRequest('production', currentOrderContext(), { requestedOrderId: id }); ordersState.productionRequestOwner=ownerFromOrderRequest(snapshot, id, 'production'); ordersState.productionLoadingOrderId=id; ordersState.productionError=''; ordersState.productionErrorInfo=null; render(); produceOrder(id, ordersState.productionNotesByOrderId[id]).then((batch)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !productionOwnerMatches(snapshot, id)) return Promise.reject({ stale:true }); ordersState.productionByOrderId[id]=batch; delete ordersState.readinessByOrderId[id]; ordersState.productionConfirmingOrderId=null; ordersState.productionError=''; ordersState.productionErrorInfo=null; ordersState.productionNotesByOrderId[id]=''; return Promise.all([getOrders(true), getOrder(id)]); }).then((result)=>{ if (!result || !orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !productionOwnerMatches(snapshot, id)) return; const [orders, updated]=result; ordersState.items=orders.orders; ordersState.selectedOrder=updated; ordersState.productionLoadingOrderId=null; ordersState.productionRequestOwner=null; render(); }).catch((e)=>{ if ((e as {stale?: boolean})?.stale || !orderRequestCanApply(snapshot) || !productionOwnerMatches(snapshot, id)) return; ordersState.productionLoadingOrderId=null; ordersState.productionRequestOwner=null; ordersState.productionError=humanProductionError(e); ordersState.productionErrorInfo=orderOperationError(id, ordersState.productionError); render(); }); }
function editOrder(id:number){ if (orderFormMutationActive() || orderReadinessBusy(id)) return; const o=ordersState.items.find(i=>i.id===id); if(!o) return; if (ordersState.referenceLoading && ordersState.showForm && ordersState.formMode==='edit' && ordersState.form.id===id) return; advanceOrderWorkspaceContext(); orderValidation = emptyFormValidationState(); orderValidationProvenance = emptyOrderValidationProvenance(); orderRefreshWarning = ''; ordersState.selectedOrder=o; ordersState.formMode='edit'; ordersState.form=formFromOrder(o); ordersState.showForm=true; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; beginOrderReferenceLoad(true); }
function submitOrderForm(event: SubmitEvent){ event.preventDefault(); const form=event.currentTarget as HTMLFormElement; const payload=orderPayloadFromForm(form); saveOrderFormDraftFromDom(); const mode=ordersState.formMode; const editedId=ordersState.form.id; const sourceType=ordersState.form.source_type; const submitContext=currentOrderContext(); const submit=orderMutationController.beginSubmit(submitContext); if (!submit) return; orderValidation=emptyFormValidationState(); orderValidationProvenance=emptyOrderValidationProvenance(); orderRefreshWarning=''; ordersError=''; applyValidationToOrderForm(orderValidation); disableOrderMutationControls(); const request=mode==='edit' && editedId ? updateOrder(editedId, payload) : createOrder(payload); request.then((saved)=>{ if (!orderMutationController.canApplySubmit(submit, currentOrderContext())) return Promise.reject({ stale:true }); orderValidation=emptyFormValidationState(); orderValidationProvenance=emptyOrderValidationProvenance(); ordersMessage=mode==='edit'?'Заказ сохранён.':'Заказ создан.'; ordersError=''; ordersState.items = [saved, ...ordersState.items.filter((item)=>item.id!==saved.id)]; ordersState.selectedOrder=saved; ordersState.showForm=false; orderMutationController.finishSubmit(submit); advanceOrderWorkspaceContext(); restoreOrderMutationControls(); render(); const refreshContext=currentOrderContext(); const refresh=orderMutationController.beginRequest('postSaveRefresh', refreshContext, { savedOrderId: saved.id }); return getOrders(true).then((response)=>{ if (!orderMutationController.canApplyPostSaveRefresh(refresh, currentOrderContext())) return; ordersState.items=response.orders; orderRefreshWarning=''; render(); }).catch(()=>{ if (!orderMutationController.canApplyPostSaveRefresh(refresh, currentOrderContext())) return; orderRefreshWarning='Заказ сохранён, но список не удалось обновить автоматически.'; render(); }); }).catch((e)=>{ if ((e as {stale?: boolean})?.stale || !orderMutationController.canApplySubmit(submit, currentOrderContext())) return; orderMutationController.finishSubmit(submit); ordersMessage=''; orderRefreshWarning=''; ordersState.formMode=mode; ordersState.form.id=editedId; ordersState.showForm=true; ordersError=''; const normalized=normalizeOrderValidation(apiValidationPayload(e), sourceType); orderValidation=normalized.validation; orderValidationProvenance=normalized.provenance; orderMutationController.markTargetedValidationApplied(); applyValidationToOrderForm(orderValidation); restoreOrderMutationControls(); announceAssertive('Проверьте заказ. Ошибки показаны рядом с полями.'); }); }
function cancelOrder(id:number){ if (orderFormMutationActive() || orderReadinessBusy(id)) return; if(!window.confirm('Отменить заказ? Заказ останется в истории, склад и производство не будут затронуты.')) return; cancelOrderRequest(id).then((saved)=>{ ordersMessage='Заказ отменён.'; ordersError=''; ordersState.selectedOrder=saved; return getOrders(true); }).then(r=>{ ordersState.items=r.orders; render(); }).catch((e)=>{ ordersError=humanOrderError(e); render(); }); }
function archiveOrder(id:number){ if (orderFormMutationActive() || orderReadinessBusy(id)) return; if(!window.confirm('Архивировать заказ? Заказ не будет удалён и останется в истории.')) return; archiveOrderRequest(id).then((saved)=>{ ordersMessage='Заказ архивирован.'; ordersError=''; ordersState.selectedOrder=saved; return getOrders(true); }).then(r=>{ ordersState.items=r.orders; render(); }).catch((e)=>{ ordersError=humanOrderError(e); render(); }); }
