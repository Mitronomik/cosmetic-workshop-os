...
- **C3 — AuditLog workspace: `INCOMPLETE`.**
  ...
  - `C3-II-B2 — JSON export AuditLog coverage`: **`IMPLEMENTED ON PR BRANCH — NOT MERGED`**. One bounded implementation PR, reusing the existing `artifact_audit_operations` ledger with no new migration, and carrying the accepted `CR-006` create-response correction. Full scope: `docs/implementation-plan.md` § *C3-II-B2*.
