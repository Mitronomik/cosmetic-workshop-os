...
Slice A4 responsive containment is DONE. The final product-owner exact-main cross-route smoke against `490b9506158c2a26d1fab9c85d0ba9f4c904cbae` returned `PASS — A4 CROSS-ROUTE EXACT-MAIN REGRESSION PASSED` for `/ingredient-lots`, `/orders`, `/clients`, `/inventory`, `/packaging-items`, passive `/ingredients`, passive `/recipes`, and passive `/client-recipes`. This is external product-owner evidence, not GitHub Actions evidence.

Slice A5 is ACTIVE. The current focused runtime task is **A5 — Human-readable local artifacts and data-location presentation** for `/backups`, `/exports`, `/report-documents`, and the directly related local-data presentation on `/settings`. The goal is to make locally generated files and the application data folder understandable to a nontechnical user while preserving local-first behavior. Full absolute paths remain available as secondary technical support information; backend storage behavior is unchanged. Restore and arbitrary filesystem browsing remain unimplemented and out of scope.
