...
Статус: `IN PROGRESS — A3.7 Orders structured validation implemented in open PR; external exact-head browser smoke required before merge`
...
Slice A3 as a whole remains IN PROGRESS. **A3.7 — Orders structured validation** is implemented in the current focused runtime pull request and remains open until review, merge, and external exact-head browser smoke. A3.7 is limited to existing Order create/edit form validation and safe mutation lifecycle.

Production Readiness and Production Confirmation remain separate future tasks and must not be silently combined with A3.7. A4 responsive table containment remains blocked until the chosen A3 closure point.
