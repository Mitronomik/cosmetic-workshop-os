...
`C1 — COMPLETED`. `C2 — COMPLETED`. `C3-I`, `C3-II-A`, `C3-II-B1`, `C3-II-B2` and `C3-II-B3` are all `DONE — MERGED AND EXACT-HEAD VERIFIED`. `CR-004`, `CR-006` and `CR-009 — ACCEPTED AND IMPLEMENTED`. `C3 artifact-finalization hardening — DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #168). `C3 — COMPLETED — MERGED, EXACT-HEAD VERIFIED AND HARDENED`. `CR-010 — ACCEPTED` (PR #169). `C4 — ACTIVE`. `C4 product decision — COMPLETE`. `C4-I — IMPLEMENTED ON PR BRANCH — FOURTH CORRECTION APPLIED — NOT MERGED`. `C4-II — PLANNED — NOT AUTHORIZED`. `C4-III — PLANNED — NOT AUTHORIZED`. `Restore — NOT IMPLEMENTED`. Product release readiness — **NOT CLAIMED**.

## Current next step

- Independently audit the new published head of PR #170 — `C4-I — Launcher-owned restore safety engine`, branch `codex/c4-i-launcher-restore-safety-engine`, started from `origin/main` at `b89cbaaaf41a56c810847d7c1e593712c5591eb6`. It is `IMPLEMENTED ON PR BRANCH — FOURTH CORRECTION APPLIED — NOT MERGED` and stays **draft** until that audit clears it.
- Do not implement `C4-II`, `C4-III`, packaging, the updater or release smoke from that branch; all remain unauthorized.
- `CR-004`, `CR-006` and `CR-009` are accepted and implemented; no change request remains in `needs evidence`.
- Restore remains **not implemented** — the `C4-I` engine is internal and has no user-facing entry point. macOS packaging, the safe packaged update flow and the full release-candidate smoke remain **not completed**.

## 2026-08-03 — C4-I fourth safety correction applied on the PR branch (not merged)

- **Trigger.** A fourth independent audit, of head `5adc215871c1becc1460d459fcf9e84ca8a24208` — the commit that closed the *third* seven findings. It found three: two safety gaps and one reporting gap. Both safety gaps are in the **boundaries** of mechanisms the third correction got right — where the lease is released, and where recovery sits in the startup order — rather than in the mechanisms themselves. All are closed on the same branch, inside the same draft pull request. **No new pull request; no merge; still draft.**
- **P1-1 — the lease covers one exact backend lifetime, not a block.** `_verify_restored_workspace()` opened a single `owned_backend_window()` around `services.startup()` *and* the whole two-cycle verifier. That left two intervals in which nothing at all held the canonical backend-liveness lock while Restore was active. **Window A:** startup initialization and migrations open and rewrite the restored working database with no backend involved, so releasing the lease there handed the workspace to nobody — a separate backend could take the lock and open the database mid-migration. **Window B:** after verification cycle 1 stopped its child the lock was free, and the launcher did not reacquire the lease before cycle 2, so a separate backend could acquire it in the gap. Startup and migrations now run under the **retained** lease, and each verification cycle is its own complete handoff — require the lease, release it, start one exact child through the pre-import entrypoint, wait for that child's handshake, run health and representative reads, stop that child by its owned handle, wait for the lock to be released, reacquire the lease in `finally`. Two cycles are therefore two releases and two reacquisitions, so the lease is held before cycle 1, **between** the cycles and after cycle 2. The invariant is stated positively: at every moment either the launcher holds the lease or one exact owned child holds the canonical lock having completed the handshake, never neither. The handoff stays implemented in exactly one place — `LauncherLifecycleContext.owned_backend_window()`, which now refuses to open unless the lease is held — and `verify_restored_backend()` takes `run_backend_cycle` as a **required keyword-only** parameter, so "release once, start twice" cannot be expressed and a caller that cannot name who owns the lease cannot start a backend at all. Failure paths are unchanged in kind and narrower in scope: the child stops, the lock release is awaited, the lease is reacquired in `finally`, and the original verification failure propagates only afterwards; a reacquisition that fails means `require_backend_stopped()` refuses, so no rollback replacement runs, the durable phase and all evidence are preserved, and startup stays blocked through the existing recovery-safe contract.
- **P1-2 — recovery is resolved before the port is checked.** `run_local_runtime()` called `assert_port_available()` before it acquired the lifecycle context and before startup recovery. That ordering was wrong about the most likely real case: a backend orphaned by a hard launcher crash is a *running backend*, so it holds the canonical liveness lock **and** it is still listening on the configured port. The port check fired first and raised `RuntimeLaunchError` about a busy port — an exception, a traceback and a message telling the user to close another window — while the typed blocked `RecoveryResult`, `RESTORE_BLOCKED_EXIT_CODE` and the fixed non-technical sentence built for exactly this case never ran. The launcher now builds its config and paths, acquires the lifecycle, resolves Restore recovery and canonical backend liveness, and only then asks whether the port is free. Ordinary collision behaviour is untouched: an unrelated program on the configured port with the canonical lock free lets recovery run normally and then fails with the same `RuntimeLaunchError` and the same user-facing port message, with no backend started and no browser opened. An occupied port is never reinterpreted as a Restore problem. Covered by a real multiprocess orphan that takes the exact canonical lock and binds and listens on the exact configured port, for both the unsafe-record and the no-record shape, driven through the real `run_local_runtime` boundary; the orphan is never signalled, and the test's cleanup terminates only the exact PID it created.
- **P2-1 — consistent finding accounting.** The pull-request body carried an aggregate that disagreed with the enumerated history, and the third-round narrative said "six" where seven findings are enumerated. The accounting is now one number, consistent everywhere: **twenty findings across four independent audits, 5 + 5 + 7 + 3**.
- **Unchanged by this correction.** The exact twelve phases, the exact transition graph, `phase` as the sole authoritative lifecycle field, launcher-owned Restore, the existing maintenance-lease primitive, pre-import backend lock acquisition, the exact-child pipe-and-token handshake, typed startup recovery, operation-ID handling, `recovery_blocked` protection, truthful completion messaging, local-first operation. No new lock file, no second state machine, no lifecycle boolean on the operation record, no PID-file authority, no discovery by port, name or command line, no orphan is killed, no supervisor, daemon, queue or background worker. No frontend Restore behaviour, no Restore API endpoint, no migration, no dependency change, no Restore AuditLog event. `launcher/restore/phases.py` untouched; `scripts/restore_backup.sh` untouched.
- **Evidence.** All `2327` node IDs of `5adc215` preserved, `0` lost. Collection `2327` → `2351`. Complete `python3 -m pytest backend/app/tests launcher/tests` → **`2351 passed`**. Launcher suite **`486 passed`**. `git diff --check` clean.
- **Not claimed.** `C4-II` and `C4-III` remain `PLANNED — NOT AUTHORIZED`. Restore remains not implemented. **Product release readiness is not claimed**, the pull request is not ready to merge, and the next action is a fifth independent audit of the new published head.

## 2026-08-03 — C4-I third safety correction applied on the PR branch (not merged)

- **Trigger.** A third independent audit, of head `606b21918ef4156cb271ca819de4e27279d49cc2` — the commit that closed the *second* five findings. It found seven findings — four safety gaps and three reporting gaps — all in code the earlier rounds had introduced or left standing. All are closed on the same branch, inside the same draft pull request. **No new pull request; no merge; still draft.**
- **P1-1 — retained backend exclusion.** The backend-liveness lock was opened, `flock`ed and released immediately. That proves availability at an instant and reserves nothing, so a backend starting one millisecond later would hold the database through journal settlement and the replacement itself while every individual check reported success. The launcher now takes that same canonical lock and **keeps** it — `BackendMaintenanceLease` — through the safety copy, journal settlement, replacement-artifact preparation, working-database replacement, rollback replacement and post-replacement verification. `require_backend_stopped()` requires the lease to still be held; a released lease is a refusal even with an intact stop proof and no backend running. The lease is released for exactly one reason, an owned backend must run, and reacquired before anything continues or rolls back; a failed reacquisition blocks recovery rather than replacing. The lock was also acquired **too late**: taking it in the FastAPI lifespan leaves the whole import of uvicorn, `app.main` and the database layer as a window in which a launcher-managed child holds nothing, so a hard launcher death there left a delayed child that could open the database underneath the next launcher. Launcher-managed children now run `python -m app.launcher_backend_entrypoint`, which acquires the lock **before importing any application module** and exits without reaching `app.main` when it cannot; the lifespan acquisition stays as an idempotent defence. Owning a `Popen` still did not prove the lock, so the child reports its acquisition over a pipe and token created for that one spawn, and `start_owned_backend_process` returns only on that exact report — timeout, EOF, an early exit and a foreign token all stop the child, and stale evidence cannot satisfy a later start. No PID file, no port, no name matching.
- **P1-2 — typed startup-recovery blocking.** `recover_incomplete_restore()` called `context.stop_backend()` directly, so an orphaned backend made `RestoreLifecycleError` escape into `run_local_runtime()`, which expects a `RecoveryResult` — turning an entirely expected condition of the gate into an unhandled exception and a traceback on the user's screen. Startup recovery now returns a typed blocked result: nothing starts, the browser never opens, the working database and the operation record are untouched, no transition is attempted, the launcher returns the existing `RESTORE_BLOCKED_EXIT_CODE`, and the technical detail stays in the local log. This holds both with an interrupted Restore record present and with no record at all.
- **P1-3 — ambiguous operation identity.** When the initial `prepared` publication reported "the rename may have landed", the engine re-read `operation.json` without checking whose record it is. If the rename in fact never landed, what it found was the **previous** operation's terminal record — so an attempt that never staged a byte could be reported as `completed`, carrying another operation's identity. The operation ID is now compared first. A foreign record is never inherited, never transitioned and never modified; the result is `aborted` with the fixed `PREPARATION_NOT_PUBLISHED` sentence, keeps *this* attempt's operation ID, cleans only this attempt's own freshly created empty directory, and lets ordinary startup proceed only after the existing record's durability is re-proved. `recovery_blocked`, live and unreadable previous records block.
- **P1-4 — `recovery_blocked` is not replaceable.** `create()` grouped all four terminal phases together, so an ordinary new attempt could overwrite the authoritative pointer to a blocked recovery and the evidence it names. Only the positive `SAFE_TERMINAL_STARTUP_PHASES` vocabulary — `completed`, `aborted`, `rolled_back` — is replaceable now; clearing a blocked recovery is a separately authorized support procedure, outside this slice.
- **P2-1 — truthful unconfirmed-completion message.** A visible-but-unconfirmed `completed` means the restored data are in place, verified, and authoritative, with only the flush that would make the rename survive a host interruption unproved. Nothing was rolled back and nothing was lost — yet the message said *"Восстановление не завершилось. Возвращены предыдущие данные мастерской."*, claiming both. One fixed `COMPLETION_DURABILITY_UNCONFIRMED` category now says only what is known: the data were restored and verified, the technical finalization could not be completed safely, all data are preserved, and reopening the application retries the confirmation. It is not a phase and not a transition — `phase` stays `completed`, `restore_succeeded` stays false while startup is refused, and the next start reports an ordinary success once confirmation succeeds.
- **P2-3 — restored test node ID.** `launcher/tests/test_restore_phases.py::test_unsafe_phases_never_permit_ordinary_startup` was restored with a corrected meaning: `UNSAFE_STARTUP_PHASES` keeps its original architectural purpose and every member is refused, without the old false converse claim that everything outside the set is safe to start from.
- **Unchanged by this correction.** The exact twelve phases, the exact transition graph, `phase` as the sole authoritative lifecycle field, launcher-owned Restore, source immutability, safety-copy-before-replacement, the rollback and startup-recovery contracts, local-first operation. No new phase. No frontend Restore behaviour, no Restore API endpoint, no migration, no dependency change, no Restore AuditLog event, no daemon, supervisor, queue or outbox. `scripts/restore_backup.sh` untouched.
- **Evidence.** All `2254` node IDs of `606b219` preserved, `0` lost. The named `2bf53e7` node restored. Collection `2254` → `2327`. Complete `python3 -m pytest backend/app/tests launcher/tests` → **`2327 passed`**. Launcher suite **`462 passed`**. `git diff --check` clean.
- **Not claimed.** `C4-II` and `C4-III` remain `PLANNED — NOT AUTHORIZED`. Restore remains not implemented. **Product release readiness is not claimed**, the pull request is not ready to merge, and the next action is a fourth independent audit of the new published head.

## 2026-08-02 — C4-I second safety correction applied on the PR branch (not merged)

- **Trigger.** A second independent audit, this time of head `2bf53e747afe5f88a7835139c146d480705f6e1f` — the commit that closed the *first* five findings. It found five more, all in code that correction had introduced or left standing. All five are closed on the same branch, inside the same draft pull request. **No new pull request; no merge; still draft.**
- **P1-1 — terminal `completed` publication.** A post-rename flush failure at `completed` left the record visible on disk and then fell through the generic publication-failure handler to `_abort()`, attempting `completed → aborted` — an edge the accepted graph does not contain — which would raise `PhaseTransitionError` out of the launcher and, worse, roll back a database that had already been restored and verified. Terminal records are now never transitioned again, not even to themselves. A dedicated terminal handler re-reads the record, confirms its durability without changing its content, and returns; `recovery_blocked` blocks unconditionally. The same handling covers `aborted` and `rolled_back`.
- **P1-2 — startup permission and `prepared` ambiguity.** Permission was derived negatively (`phase not in UNSAFE_STARTUP_PHASES`), which silently admitted the four *unresolved* pre-replacement phases: safe for the database, because replacement never happened, and unsafe for startup, because the operation is still live. It is now one positive allow-list — `permits_ordinary_startup(durable_phase, record_exists, durability_confirmed)` — used by `RestoreResult`, `RecoveryResult`, execution failure handling, startup recovery and the browser gate. Only `completed`, `aborted` and `rolled_back` qualify, and only after the record's durability is confirmed; no record at all still permits startup; an unreadable record never does. Separately, `store.create()` can rename and then fail its flush, leaving a `prepared` record on disk; that is no longer reported as `durable_phase=None`, but re-read and either closed as `aborted` or left blocking for the next start.
- **P1-3 — same-size in-place source modification.** Device, inode, size and file type all stay identical when a writer rewrites bytes in place, so the previous identity check could not see it and a mixed candidate — pages from two source states, individually well-formed and `quick_check`-clean — could be published. `SourceIdentity` gained `st_mtime_ns` and `st_ctime_ns`, and staging now proves content stability with **two independent SHA-256 passes** over the one held descriptor: the first hashes exactly the bytes written into the staging scratch, the second re-reads from offset zero and hashes without writing, with stat/path/sidecar revalidation around both. The digest comparison is the load-bearing half, because it does not depend on filesystem timestamp resolution.
- **P2-1 — orphaned backend after a hard launcher crash.** `BackendProcessOwner` held a `Popen` in launcher memory; a `SIGKILL`ed launcher lost it while the uvicorn child kept running and kept the database open, and the next launcher would have concluded that nothing was running. The backend process now holds a dedicated **backend-liveness lock** for its whole lifetime, taken in its FastAPI lifespan from the path the launcher passes in `COSMETIC_WORKSHOP_BACKEND_LIVENESS_LOCK` and derived from the same canonical workspace. The kernel releases it on process death for any reason, so a held lock means a live backend whoever started it. `stop_backend()` stops the owned handle, waits for release, then proves the lock free; `require_backend_stopped()` re-checks both before journal settlement and every replacement including rollback. **An orphan is detected, never killed** — no PID file, port, name or pattern lookup — and Restore, startup recovery, database replacement, second backends and the browser are all refused while one is alive.
- **P2-2 — durability method evidence.** `flush_file()` returned a `FlushMethod` that production callers dropped, while the documentation claimed the fallback was "recorded". It now is: every safety-critical flush records its publication category, target kind (file or directory), platform and method into `DURABILITY_DIAGNOSTICS` and the technical log. Categories cover the operation record, existing-record confirmation, staged candidate, replacement artifact, working-database replacement and rollback replacement; a directory flush is recorded separately as `directory_fsync` and never conflated with a full-device flush; an unexpected I/O error still propagates rather than being downgraded. The method is deliberately **not** added to the authoritative operation record — it is evidence about a write, not a lifecycle fact.
- **Also corrected.** `restore_succeeded` now additionally requires `normal_startup_allowed`: claiming success while refusing to start the application is a mixed message a future `C4-II` screen cannot render honestly.
- **Unchanged by this correction.** The exact twelve phases, the exact transition graph, `phase` as the sole authoritative lifecycle field, launcher ownership, the candidate-validation and safety-copy contracts, local-first operation. No new phase was added — `completed`-but-unconfirmed, `durability_failed` and `backend_orphaned` are transient result/diagnostic states. No Restore API endpoint, no frontend change, no migration, no dependency change, no Restore AuditLog event, no generic process supervisor or daemon, no `C4-II`/`C4-III` work. `scripts/restore_backup.sh` untouched.
- **Evidence.** Original pre-`C4-I` baseline `1843` node IDs: **all preserved, 0 lost**. Collection `2164` → `2254`. Complete `python3 -m pytest backend/app/tests launcher/tests` → **`2254 passed`**. Launcher suite **`400 passed`**. `git diff --check` clean. One intermediate test of this branch's own, `test_unsafe_phases_never_permit_ordinary_startup`, was **split rather than deleted**: its subject — the negative permission rule — is false by design after P1-2, and its unsafe-set assertion is preserved verbatim in `test_the_unsafe_phase_set_is_unchanged` alongside new per-phase permission tests.
- **Not claimed.** `C4-II` and `C4-III` remain `PLANNED — NOT AUTHORIZED`. Restore remains not implemented. **Product release readiness is not claimed**, the pull request is not ready to merge, and the next action is a third independent audit of the new published head.

## 2026-08-02 — C4-I safety corrections applied on the PR branch (not merged)

- **Trigger.** An independent review of PR #170's first published head `a66ddd6c34f9790cee4fedad7461cd41f9fdc154` found five safety-critical implementation gaps. The structure and the accepted twelve-phase vocabulary were sound; the safety contract was not yet proved. All five are closed on the same branch, inside the same draft pull request. **No new pull request; no merge; still draft.**
- **P1-1 — source WAL/journal safety.** Sidecars are now checked beside the **original selected source** (`<source>-wal`, `-shm`, `-journal`, by `lexists` so a symlinked or dangling one counts), before the copy and again after it, before `candidate.sqlite` is published. The previous check looked only beside the already-staged candidate, which by construction could never see them — so a source with committed rows still in its WAL would have been staged incomplete while `quick_check` returned `ok`. The source is opened once with `O_RDONLY | O_NOFOLLOW` and the copy reads that **held descriptor** via `os.pread`; identity is re-proved from the descriptor (`fstat`: device, inode, size, type) and from the path (`lstat`: same file, not a symlink), and the copied byte count must equal the recorded size. A hot rollback journal is rejected **before** the file is opened, because opening it would roll the journal back — a write to the user's file. Tests use real WAL and real hot-journal sources produced by killed writer processes.
- **P1-2 — backend shutdown proof.** New `LauncherLifecycleContext` + `BackendProcessOwner`. `run_local_runtime` hands the child it starts to the context, so the launcher owns the exact `Popen`. Restore stops it before anything touches the working database: `SIGTERM`, bounded wait, escalation to `kill()` **on the same handle**, wait again, and a `BackendStopProof` only once `poll()` shows it exited. Re-checked before journal settlement and before every replacement including rollback. No `pkill`, `lsof`, `pgrep` or `killall` — enforced by a test over the module's executable source. A real-uvicorn integration test proves the exact recorded PID is dead before journal settlement.
- **P1-3 — canonical destructive paths.** `RestoreRequest` now has **one field**, `selected_source`. The database path, backup directory, Restore directory and lock path are derived by the context from `startup_database_config` and `resolve_backup_dir`; `verify_derived_paths()` re-derives and compares, so a mutated context is refused before staging. The self-comparison `assert_replaceable_target(x, x)` — which passed for every path in the filesystem — is gone; the target is re-resolved from the startup resolver. Development mode stays isolated from the real Documents directory.
- **P1-4 — rollback publication failures.** A failed `rollback_in_progress` publication no longer attempts `replacement_intent → recovery_blocked`, an edge the accepted graph does not contain and which raised `PhaseTransitionError` out of the launcher. The actual durable phase is re-read and reported, ordinary startup is blocked, evidence is preserved, and the next launcher start retries from that real phase. Results now carry `outcome`, `durable_phase` and `normal_startup_allowed` separately, and `restore_succeeded` is keyed off the durable phase so an unpersisted outcome cannot claim success.
- **P1-5 — durability boundary.** One shared `launcher/restore/durability.py` publishes the operation record, the staged candidate and both replacements. The parent-directory flush is **mandatory**, not best effort. Failures are classified `BEFORE_REPLACE` / `DURING_REPLACE` / `AFTER_REPLACE`; a post-rename failure is reported as possibly-published, so state callers re-read and a database replacement enters rollback. macOS uses `F_FULLFSYNC` where the filesystem supports it and **records the fallback** when it does not, so no stronger guarantee is claimed than the call that ran. No power-loss guarantee is claimed anywhere.
- **P2 hardening.** Safety-copy verification now reuses `validate_workspace_snapshot`, the same read-only contract a Restore candidate passes (identity tables and required tables included). The repository fallback database is fingerprinted with **SHA-256 + size + stat identity** instead of existence/size/mtime. The replacement artifact has a **deterministic** name derived from the operation ID, so recovery removes exactly the one file it owns with no globbing beside the user's database. Operation IDs must be canonical launcher-generated UUID4s, not merely safe filenames.
- **Smoke runner removed from the PR.** `scripts/c4_i_restore_smoke.sh` and `scripts/c4_i_restore_smoke.py` were deleted: a PR-specific exact-head runner must live outside the pull request it verifies. The runner is now created outside the repository and drives a detached worktree at the exact published head, records only PIDs it started, terminates only those PIDs, and classifies its result as `PASS — FULL AUTOMATED SMOKE PASSED`, `FAIL — PRODUCT`, `INCONCLUSIVE — RUNNER` or `INCONCLUSIVE — ENVIRONMENT`. `scripts/restore_backup.sh` is unchanged.
- **Unchanged by the correction.** The exact twelve phases, the exact transition graph, `phase` as the sole authoritative lifecycle field, launcher ownership, local-first operation. No Restore API endpoint, no frontend change, no migration, no Restore AuditLog event, no generic workflow framework, no `C4-II` or `C4-III` work.
- **Evidence.** Baseline collection `1843` node IDs; final `2164`; **`1843` preserved, `0` lost, `321` added**. Complete `python3 -m pytest backend/app/tests launcher/tests` → `2164 passed`. `git diff --check` clean. Frontend untouched; no migration; no dependency change.
- **Not claimed.** `C4-II` and `C4-III` remain `PLANNED — NOT AUTHORIZED`. Restore remains not implemented. **Product release readiness is not claimed**, and the pull request is not ready to merge — the next action is an independent audit of the new published head.

## 2026-08-02 — C4-I implemented on its PR branch (not merged)

- **Slice.** `C4-I — Launcher-owned restore safety engine`, the only runtime slice authorized by `CR-010`. Branch `codex/c4-i-launcher-restore-safety-engine`, based on `origin/main` = `b89cbaaaf41a56c810847d7c1e593712c5591eb6` (the PR #169 merge commit, verified as an ancestor before any edit). Clean worktree including untracked files at start; zero open pull requests and no conflicting Restore branch.
- **What it is.** Internal launcher infrastructure for a future `C4-II` flow, callable as `execute_restore(...)` and `recover_incomplete_restore(...)`. **No** Restore API endpoint, button, dialog, file picker, frontend route, product terminal workflow, AuditLog event, migration, schema change or dependency change. **Restore stays `NOT IMPLEMENTED` as a product capability.**
- **Structure.** `launcher/restore/` as a focused package: `phases` (twelve phases and the complete transition graph), `contracts` (typed request/result/outcome types and the fixed Russian message set), `state` (durable record and atomic publication), `workspace` (isolated operation directory and ownership), `instance_lock` (`fcntl.flock`), `staging`, `validation`, `capacity`, `safety_copy`, `replacement`, `verification`, `engine`, `recovery`. One bounded backend addition: `backend/app/db/migration_lineage.py`, a read-only lineage and required-table helper that never creates the migration table. `launcher/runtime.py` gained the instance lock, the pre-startup recovery gate and the browser gate.
- **Accepted state machine implemented exactly.** The same twelve lowercase-ASCII phases, the same transition graph, the same four terminal phases, the same recovery matrix and the same `replacement_intent` crash rule as ADR 0016 § 7. `phase` is the sole authoritative lifecycle field; no independent replacement/rollback boolean is persisted. No deviation was required, so no amending decision was written.
- **Durable state.** One narrow record at `<user data base>/restore/operation.json`, outside the SQLite working database, the repository, the package and frontend storage. Allowed fields exactly: `operation_id`, `phase`, `created_at`, `updated_at`, `staged_candidate_filename`, `safety_copy_filename`. Reading rejects any other field set. No raw absolute source path is stored. Publication is `mkstemp(O_EXCL)` → write → flush → `os.fsync` → `os.replace` → best-effort parent-directory `fsync`, never an in-place rewrite; **no power-loss durability is claimed** on macOS, only old-or-new atomicity across process and OS crash.
- **Safety gates.** Conservative per-device disk-space preflight before any large artifact (formula in `docs/backup-and-restore.md` § 16.5); mandatory verified `before_restore` safety copy through the existing ADR 0015 SQLite Online Backup engine, with **no** `artifact_audit_operations` row and **no** AuditLog event; target WAL/SHM/journal settled through SQLite's own lifecycle and then verified, never blind-unlinked; atomic same-filesystem `os.replace` onto the exact configured database path only.
- **Verification.** Bounded readiness polling plus `GET /api/health`, `GET /api/settings/status` and `GET /api/settings/workshop-profile`, run twice with a graceful stop between — the restartability proof — against the exact restored path, with the repository fallback database fingerprinted before and after.
- **Evidence.** Baseline collection `1843` node IDs; final `2081`; **`1843` preserved, `0` lost, `238` added**, all in new files. Complete `python3 -m pytest backend/app/tests launcher/tests` → `2081 passed`. `git diff --check` clean. Developer-only isolated exact-head Restore smoke → `PASS` (successful Restore against a real backend, plus a `replacement_intent` crash recovering to `rolled_back`), repository clean before and after.
- **Not claimed.** `C4-II` and `C4-III` remain `PLANNED — NOT AUTHORIZED`. Restore remains not implemented. macOS packaging, the safe packaged update flow and the full release-candidate smoke remain not completed. **Product release readiness is not claimed.**

## 2026-08-02 — C3 closed, merged, exact-head verified and hardened; C4 Restore decided as launcher-assisted

- **Documentation-only milestone.** One documentation pull request that closes the merged lifecycle of PR #168, records C3 as fully completed and hardened, makes the C4 Restore product decision, and authorizes exactly one bounded future implementation slice. No Restore behaviour and no runtime code is changed by it.
- **Verified start gate.** Repository `Mitronomik/cosmetic-workshop-os`; clean worktree including untracked files; zero open pull requests; `origin/main` = `867afeb0967637d07172f88c95e02e9bc500a311`, exactly the PR #168 merge commit with no later commit.
- **PR #168 closure facts.** `C3 hardening — Separate artifact verification from AuditLog persistence`, state `MERGED`, base `main`, final reviewed head `6c57c7f5ba851ce2124577268baeda07d19ce4ae`, merge commit `867afeb0967637d07172f88c95e02e9bc500a311`, merged `2026-08-02T08:34:02Z`. The final reviewed head is an ancestor of the merge commit, and the merge commit is an ancestor of `origin/main`.
- **Accepted PR #168 evidence — not re-executed in this documentation PR:** complete backend `1826 passed`; complete root suite `1843 passed`; launcher `17 passed`; baseline node IDs `1804` preserved, `0` lost, `39` added; all `21` frontend `test:*` scripts passed; frontend production build `PASS`; `frontend/src/main.ts` `6399` lines; final independent audit `P0 — none`, `P1 — none`, `P2 — documented only`; exact-head launcher/API/browser smoke `PASS`.
- **Accepted hardened behaviour on merged `main`.** Report-document and JSON-export finalization use typed, artifact-specific results: `recorded` (verified, AuditLog event committed), `audit_pending` (verified, AuditLog persistence did not commit) and `artifact_invalid` (mandatory verification did not prove the artifact authoritative). Only `recorded` and `audit_pending` may produce HTTP `201`; `artifact_invalid` produces a fixed structured HTTP `500`. An invalid artifact is not deleted, is not audited, remains unresolved and is counted for bounded reconciliation. Filenames, paths, reasons, operation IDs, schema versions, entity counts, verifier details and SQLite details are not exposed through safe error responses.
- **`CR-010 — Decide launcher-assisted Restore semantics` accepted.** MVP Restore is launcher-assisted: it is not a running-backend FastAPI mutation and not an ordinary SPA mutation. The launcher owns process shutdown, backup validation, the pre-restore safety copy, staging, atomic database replacement, post-restore startup verification, rollback and incomplete-restore recovery. Support-assisted recovery stays a fallback. The user never needs Git, Python, Node.js, Docker, SQLite tools, GitHub or a terminal. Durable decision: `docs/decisions/0016-launcher-assisted-restore.md`.
- **Accepted contract highlights:** whole-database restore only from a validated local SQLite backup; the selected source is immutable read-only input; validation runs from a staged read-only copy and must complete before any mutation, replacement, deletion or migration of the current working database — the isolated operation directory, the durable operation record, launcher-owned staging files and local technical logs are Restore infrastructure, not such a mutation — and `quick_check = ok` alone is never sufficient; a newer-than-current schema is rejected before replacement while an older known schema is migrated only on the restored working copy; a verified `before_restore` safety copy through the ADR 0015 engine is mandatory; staging plus a same-directory atomic replacement boundary, with no claim that filesystem replacement and SQLite are one transaction; one narrow launcher-owned durable operation record outside the working database, recovered before the ordinary backend starts; automatic rollback after any post-replacement failure; the browser opens only after post-restore verification passes; **no Restore AuditLog event is authorized and `restore.completed` is not implicitly authorized**.
- **Accepted Restore phase machine.** The durable launcher-owned operation record carries **exactly one authoritative field, `phase`**, mutually exclusive; whether replacement occurred and whether rollback completed are **derived** from it and are never independent authoritative fields. The vocabulary is exactly twelve lowercase-ASCII values — `prepared`, `source_staged`, `candidate_validated`, `safety_copy_verified`, `replacement_intent`, `replacement_committed`, `verification_in_progress`, `completed`, `aborted`, `rollback_in_progress`, `rolled_back`, `recovery_blocked` — with no alias and no additional phase. Allowed transitions are the normal path `prepared → source_staged → candidate_validated → safety_copy_verified → replacement_intent → replacement_committed → verification_in_progress → completed`, the pre-replacement failure path to `aborted`, `replacement_intent`/`replacement_committed`/`verification_in_progress` → `rollback_in_progress`, and `rollback_in_progress` → `rolled_back` or `recovery_blocked`. Terminal phases: `completed`, `aborted`, `rolled_back`, `recovery_blocked`. **`replacement_intent` is durably recorded before the atomic replacement boundary and is always treated conservatively as though replacement may have occurred**, never resolved by inspecting timestamps, size, filenames, inode identity, migration version or working-database contents; the safe outcome is rollback from the verified safety copy. Every transition is persisted through one documented, tested, atomic publication boundary — an in-place truncate-and-rewrite of the only record is insufficient — and every persisted phase has exactly one required startup behaviour. The browser opens only after durable `completed`; `rolled_back` is a failed Restore; `recovery_blocked` never permits ordinary startup. Complete matrix: `docs/decisions/0016-launcher-assisted-restore.md` § 7.5 and `docs/backup-and-restore.md` § 7.4.
- **Subdivision:** `C4-I — Launcher-owned restore safety engine` is `AUTHORIZED AFTER THIS DOCUMENTATION PR MERGES — NOT IMPLEMENTED` and is the only runtime slice authorized. It must implement the accepted phase machine and recovery matrix exactly and may not substitute an alternative one. `C4-II — User-facing launcher Restore flow` and `C4-III — Restore end-to-end verification and lifecycle closure` are `PLANNED — NOT AUTHORIZED`. No future implementation PR number is assigned.
- **Not reopened:** `CR-004`, `CR-005`, `CR-006`, `CR-007`, `CR-008`, `CR-009` and their accepted decisions.
- **Documentation-only scope:** no backend, frontend, launcher, script, test, migration, schema, dependency, lockfile, CI, generated-file or local-user-data change. No backend suite, frontend suite, frontend build, launcher suite or Restore smoke was executed, and no runtime artifact was created; the repository contains no executable documentation or lifecycle-consistency test to run.
- **Lifecycle:** `C3 — COMPLETED — MERGED, EXACT-HEAD VERIFIED AND HARDENED`; `CR-010 — ACCEPTED — NOT IMPLEMENTED`; `C4 — ACTIVE`; `C4 product decision — COMPLETE`; `C4 implementation — NOT STARTED`; Restore not implemented; macOS packaging, safe packaged update flow and full release-candidate smoke not completed; **product release readiness is not claimed**.

## HISTORICAL — PARTIALLY SUPERSEDED — 2026-08-02 — C3 complete on merged `main`; artifact-finalization hardening implemented on its PR branch

> The implementation record below stands. Its closing lifecycle line — **Not merged** — is superseded: the hardening merged as PR #168 at `867afeb0967637d07172f88c95e02e9bc500a311`.
...
## HISTORICAL — PARTIALLY SUPERSEDED — 2026-08-02 — CR-004 resolved; C3-II-B3 implemented on its PR branch; open and unmerged

> The `CR-004` evidence and the delivered scope stand. The closing **Not merged** line is superseded: `C3-II-B3` merged as PR #167 at `7af53a3305fa9fdb984d4c478e1186685fbb6727`, and C3 is now complete and hardened on merged `main`.
...
## HISTORICAL — PARTIALLY SUPERSEDED — 2026-08-01 — C3-II-B2 implemented on its PR branch; open and unmerged

> The implementation record stands. The closing **Not merged** line is superseded: `C3-II-B2` merged as PR #166 at `844526ae4057a454312f790abcaf21be518cdbd9`.
...
## HISTORICAL — PARTIALLY SUPERSEDED — 2026-07-30 — C3-II-A closed; CR-009 accepted; C3-II-B subdivided

> The PR #161 closure facts and the accepted `CR-009` contract stand. The **Subdivision** bullet's lifecycle — B1 authorized, B2 blocked by CR-006, B3 blocked by CR-004, C3 incomplete, C4 inactive — is superseded: all three B slices merged (PR #163, PR #166, PR #167), the artifact-finalization hardening merged as PR #168, and `CR-010` has since decided C4 Restore.
