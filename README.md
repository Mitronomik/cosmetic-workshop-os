...
`R3 — Repair purchase-suggestions API smoke seeding` is the only active slice, it is **test-only**, and it is `IMPLEMENTED — REVIEW AND MERGE REQUIRED`. It changed exactly one value: `lot_qty="0"` became `lot_qty="1"` in the `seed_ready(...)` call at `backend/app/tests/test_purchase_suggestions.py:214`. `packaging_qty="2"` on that same line and the existing `minimum_stock='10'` setup on the following lines are unchanged, because that threshold is already higher than the new lot quantity of `1`, so the below-minimum condition the test needs remains true. No other line in the test changed. The test now reaches the `/api/purchase-suggestions` HTTP surface and its existing assertions — including the three no-mutation assertions — actually execute and pass. No production code changed, and its required smoke is the backend suite only. `R2 — Import draft issue-count contract alignment` remains the next deferred slice and must not start before `R3` is reviewed and merged. Contract: `state/current-focus.md`. Evidence: `docs/backend-baseline-failure-triage.md`.
