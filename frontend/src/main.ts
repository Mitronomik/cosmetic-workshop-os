...
function clientRecipeFilterToolbar(resultCount: number) {
  ...
  const clientOptions = clientRecipesState.clients.map((client) => `<option value="${client.id}" ${f.clientId === String(client.id) ? 'selected' : ''}>${escapeHtml(client.full_name)}</option>`).join('');
...
function openClientRecipeCreate() { clientRecipesState.showCreateForm = true; clientRecipesState.selectedDetail = null; clientRecipesState.detailStatus = 'idle'; clientRecipesMessage = ''; clientRecipesError = ''; render(); requestAnimationFrame(() => document.querySelector<HTMLInputElement>('[data-field="client-recipe-title"]')?.focus()); }
