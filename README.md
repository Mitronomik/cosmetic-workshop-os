...
The current focus is `C3-II-B3 — Manual backup AuditLog coverage`, **implemented on its PR branch and not merged**. C1 and C2 are **COMPLETED**. C3-I (PR #159), C3-II-A (PR #161), C3-II-B1 (PR #163) and C3-II-B2 (PR #166) are **DONE — MERGED AND EXACT-HEAD VERIFIED**. C3 remains **INCOMPLETE** on merged `main`; B3 is its last remaining slice. `CR-009` is accepted and implemented on merged `main` for report documents and JSON exports. `CR-004` is now **accepted**: raw `shutil.copy2` of the live SQLite main file silently omitted all committed-but-uncheckpointed WAL data while returning `quick_check = ok`, produced mixed transaction state including never-committed rows with the stock page cache, and in one scenario produced a structurally corrupt file — classified `PRODUCT DEFECT — BACKUP CONSISTENCY`, severity `HIGH`, with no source mutation and no live-data loss. Durable contracts: `docs/audit-log.md`, `docs/decisions/0013-file-backed-artifact-audit-semantics.md`, `docs/decisions/0014-json-export-create-confirmation-semantics.md` and `docs/decisions/0015-sqlite-backup-consistency-and-manual-audit.md`.
