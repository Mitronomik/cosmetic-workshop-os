...
function alertList(items: AlertResponse[]) { const snapshot = alertsLifecycle.state.snapshot; if (snapshot && snapshot.items.length === 0 && snapshot.filters.status === 'open' && !snapshot.filters.type && !alertsLifecycle.state.search) return `<section class="card empty-card"><h2>Открытых алертов нет</h2><p>Сейчас система не видит проблем, требующих внимания.</p><p class="next-step">Если вы изменили склад или заказы, нажмите «Обновить алерты».</p></section>`; if (items.length === 0) return `<section class="card empty-card"><h2>По этим фильтрам алертов нет</h2><p>Измените фильтр или сбросьте поиск.</p><button class="secondary-action" type="button" data-action="reset-alert-filters">Сбросить фильтры</button></section>`; return `<section class="card data-card" id="alerts-list-region" tabindex="-1"><p class="card-kicker">Список</p><h2>Алерты мастерской</h2><div class="recipe-lines">${items.map(alertCard).join('')}</div></section>`; }
...
function loadAlerts(force = false, requestedFilters?: AlertFilters, kind?: 'initial'|'refresh'|'filter'|'regeneration-refresh') { const filters = requestedFilters ?? alertsLifecycle.displayedFilters(); const readKind = kind ?? (!alertsLifecycle.state.snapshot ? 'initial' : 'refresh'); if (!force && alertsLifecycle.state.snapshot) return; const ownerGeneration = alertsLifecycle.state.routeGeneration; const started = alertsLifecycle.startRead(readKind, filters, ownerGeneration); if (!started.accepted) return; if (readKind !== 'initial') clearFeedbackAnnouncement(); render(); getAlerts(started.filters as AlertsState['filters']).then((response) => { const result = alertsLifecycle.finishReadSuccess(started.requestId, started.routeGeneration, started.filters, response.alerts); if (result.present) { render(); if (result.announcement === 'polite') announcePolite(result.message); } }).catch(() => { const result = alertsLifecycle.finishReadFailure(started.requestId, started.routeGeneration, started.filters); if (result.present) { render(); if (result.announcement === 'assertive') announceAssertive(result.message); } }); }
function finishAlertMutationPresentation(result: { present: boolean; announcement: 'none'|'polite'|'assertive'; message: string; focusKey: string | null }) { if (!result.present) return; render(); if (result.announcement === 'polite') announcePolite(result.message); if (result.announcement === 'assertive') announceAssertive(result.message); if (result.focusKey) requestAnimationFrame(() => focusAlertKey(result.focusKey!)); }
...
function runAlertAction(id: number, action: 'resolve' | 'dismiss') { const previousFocusKey = `alert-${id}-${action}`; const started = alertsLifecycle.startMutation(action, alertsLifecycle.state.routeGeneration, id, previousFocusKey); if (!started.accepted) return; clearFeedbackAnnouncement(); render(); const request = action === 'resolve' ? resolveAlert(id) : dismissAlert(id); request.then((response) => { const result = alertsLifecycle.applyMutationResponse(started.requestId, started.routeGeneration, id, action, response); if (!result.present) return; render(); if (result.announcement === 'polite') announcePolite(result.message); if (result.announcement === 'assertive') announceAssertive(result.message); const focusKey = selectAlertFocusTarget(result.present, previousFocusKey, id, alertFocusCandidates(previousFocusKey, id)); if (focusKey) requestAnimationFrame(() => focusAlertKey(focusKey)); }).catch(() => { finishAlertMutationPresentation(alertsLifecycle.finishMutationFailure(started.requestId, started.routeGeneration)); }); }
function runAlertRegeneration() { const started = alertsLifecycle.startMutation('regenerate', alertsLifecycle.state.routeGeneration); if (!started.accepted) return; clearFeedbackAnnouncement(); render(); regenerateAlerts().then((result) => { const mutationResult = alertsLifecycle.finishRegenerationSuccess(started.requestId, started.routeGeneration, result); if (!mutationResult.accepted) return; const filters = alertsLifecycle.displayedFilters(); const refresh = alertsLifecycle.startRead('regeneration-refresh', filters, started.routeGeneration); if (!refresh.accepted) { finishAlertMutationPresentation(mutationResult); return; } return getAlerts(refresh.filters as AlertsState['filters']).then((response) => { alertsLifecycle.finishReadSuccess(refresh.requestId, refresh.routeGeneration, refresh.filters, response.alerts); finishAlertMutationPresentation({ ...mutationResult, focusKey: selectAlertFocusTarget(mutationResult.present, 'regenerate-alerts', null, [{ key: 'regenerate-alerts', kind: 'action', enabled: true, attached: true }, { key: 'alerts-main-heading', kind: 'main-heading', enabled: true, attached: true }]) }); }).catch(() => { const refreshResult = alertsLifecycle.finishReadFailure(refresh.requestId, refresh.routeGeneration, refresh.filters); const message = refreshResult.present ? ALERT_REGENERATION_REFRESH_WARNING_ANNOUNCEMENT : mutationResult.message; finishAlertMutationPresentation({ ...mutationResult, announcement: refreshResult.present ? 'assertive' : mutationResult.announcement, message, focusKey: 'alerts-main-heading' }); }); }).catch(() => { finishAlertMutationPresentation(alertsLifecycle.finishMutationFailure(started.requestId, started.routeGeneration)); }); }
