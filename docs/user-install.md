...
- если обычное двойное нажатие не разрешает запуск, нажмите на `CosmeticWorkshopOS.app` правой кнопкой мыши или Control-click и выберите **Open / Открыть**;
...
Закрытие вкладки или окна браузера **не означает**, что локальное приложение остановлено.

1. Найдите значок «Мастерской косметолога» в Dock.
2. Вызовите меню значка правой кнопкой мыши или Control-click.
3. Выберите **Quit / Завершить** и дождитесь, пока значок приложения перестанет показывать запущенное состояние.
4. Снова откройте тот же `CosmeticWorkshopOS.app` через Finder.
...
- Если браузер не открыл рабочий интерфейс — завершите приложение через Dock → **Quit / Завершить** и сообщите сопровождающему.
