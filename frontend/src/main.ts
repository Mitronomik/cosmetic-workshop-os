...
type ProductionReadinessIngredientLine = { ingredient_id: number; ingredient_name: string; required_quantity: string; required_unit: string; available_quantity: string; missing_quantity: string; can_fulfill: boolean; selected_lots: ProductionReadinessLotSelection[]; warnings: ProductionReadinessIssue[] };
...
let ordersError = '';
let ordersMessage = '';
let ordersState: OrdersState = { items: [], clients: [], templates: [], versions: [], clientRecipes: [], packagingItems: [], formMode: 'create', form: emptyOrderForm(), showForm: false, selectedOrder: null, includeInactive: true, filters: { search: '', status: 'active' }, readinessByOrderId: {}, readinessLoadingOrderId: null, readinessError: '' };
...
function readinessNextAction(status: ProductionReadinessStatus) { return status === 'blocked' ? 'Сначала устраните препятствия: добавьте недостающие партии, тару или исправьте данные рецепта.' : status === 'warning' ? 'Проверьте предупреждения перед производством. Фактическое производство и списание склада появятся отдельным шагом.' : 'Компонентов и тары хватает. Фактическое производство и списание склада появятся отдельным шагом.'; }
...
function readinessEstimates(result: ProductionReadinessResponse) { return `<div class="readiness-block"><h3>Предварительная экономика</h3><div class="readiness-grid"><div><strong>Ориентировочная себестоимость</strong><p>${moneyOrMissing(result.estimated_cost)}</p></div><div><strong>Налог</strong><p>${moneyOrMissing(result.estimated_tax)}</p></div><div><strong>Маржа</strong><p>${moneyOrMissing(result.estimated_margin)}</p></div></div><p class="next-step">Если налог или маржа не рассчитаны, причина указана в предупреждениях backend. Интерфейс не подставляет налоговую ставку сам.</p></div>`; }
...
function checkOrderReadiness(id:number){ const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; if(!order || order.status==='cancelled' || order.status==='archived' || !order.is_active) return; ordersState.readinessLoadingOrderId=id; ordersState.readinessError=''; render(); checkOrderProductionReadiness(id).then((result)=>{ ordersState.readinessByOrderId[id]=result; ordersState.readinessLoadingOrderId=null; ordersState.readinessError=''; render(); }).catch((e)=>{ ordersState.readinessLoadingOrderId=null; const msg=e instanceof Error && e.message && e.message !== 'API request failed' ? e.message : ''; ordersState.readinessError=msg || 'Не удалось проверить готовность производства.'; render(); }); }
