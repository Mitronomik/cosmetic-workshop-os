...
Статус: `IN PROGRESS — A3.2 inventory structured validation closure`
...
Current sub-slice A3.2 / PR116 covers only existing inventory frontend forms: `/stock-movements` manual ingredient-lot movement create, `/packaging-items` Packaging Item create, and `/packaging-items` Packaging Item edit. The current frontend Stock Movement route does not expose packaging movements; packaging movement UI remains follow-up work even though backend packaging movement APIs exist.
