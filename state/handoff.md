...
## Last completed work
Implemented PR2 database, migrations, settings and audit foundation. The backend now has SQLite connection/session helpers, a small migration runner, an initial infrastructure migration, seeded app-level settings, audit log table structure, and thin technical endpoints for `/api/database/status` and `/api/settings`. Existing `/health` and `/api/health` response shape is preserved.
...
- Default local development DB path is `.local/cosmetic_workshop.sqlite`, which is gitignored. This is intentionally not the final user data directory behavior.
...
- `PYTHONPATH=backend python3 - <<'PY' ...` temporary SQLite migration smoke
- `git status --short`
