...
**Current merged baseline.** `origin/main` is `d432fcaee52a16a4f8b609ec160cf3fa2b33d013` (the PR #154 merge commit), and the current accepted merged backend result is `883 passed / 0 failed / 0 skipped`. The earlier value `c3a3a7b8db06fe85290216113b784123ed9b6b30` was the PR #152 merge commit and is **historical**.
...
The current focus is the **C2 financial implementation sequence**. `R3`, `R2`, and `R4` are all merged and DONE, the backend baseline correction gate is **DONE**, `CR-007` is accepted, and `C1-I` is merged and **DONE**. `CR-008` records the accepted C2 calculation and immutable-snapshot contract; `C2-I` (PR #151), `C2-II` (PR #152) and `C2-III-A` (PR #154) are merged and `DONE`, and `main` is now `d432fcaee52a16a4f8b609ec160cf3fa2b33d013`. **No runtime implementation slice is active.** The former `C2-III` umbrella is subdivided into `C2-III-A` (Order and `ProductionBatch` financial presentation — merged and `DONE`) and `C2-III-B` (snapshot-backed reports and report documents — `AUTHORIZED AFTER THIS DOCUMENTATION PR MERGES — NOT IMPLEMENTED`). Reports are still not snapshot-backed. C2 is **not complete**.
...
`R4 — Canonical backup/export filename reason normalization` delivered that contract as one bounded slice covering both nodes through a single shared backend helper, and it is **merged and DONE** as PR #146. `CR-006` is a `needs evidence` row and is **not active**. `CR-004` remains separate and inactive. C1 has an accepted product contract (`CR-007`) and a merged implementation (`C1-I`, PR #149), so C1 is **DONE**. C2 has an accepted product contract (`CR-008`) and a **partial implementation**: `C2-I`, `C2-II` and `C2-III-A` are merged, and `C2-III-B` is authorized after this documentation PR merges and is not implemented, so C2 is **not complete**. C3 and C4 remain inactive. Restore remains open, packaging/install/update work and the full release-candidate smoke remain open, and **product release readiness is not claimed**. Contract: `state/current-focus.md` and `docs/implementation-plan.md`. Evidence: `docs/backend-baseline-failure-triage.md`.
