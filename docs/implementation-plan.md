...
Статус: `IN PROGRESS — A3.8 Production Readiness feedback and lifecycle is the current focused runtime slice`
...
Slice A3 as a whole remains IN PROGRESS. **A3.8 — Production Readiness feedback and lifecycle** is the current focused runtime slice. Scope is limited to the existing read-only readiness result, accessible loading and system-failure feedback, duplicate execution prevention, current-order request ownership/freshness, contextual issue presentation, focused frontend lifecycle tests, and no-write readiness regression tests.

**A3.9 — Production Confirmation** remains separate. A3.8 must not change `POST /produce`, production confirmation behavior, stock write-off, production batches, order lifecycle rules, FEFO calculations, cost/tax/margin rules, schemas, migrations, responsive table containment, or unrelated routes. A4 remains separate and blocked until the chosen A3 closure point.
