...
function loadSectionData(section: NavigationSection) {
  ...
  if (section === 'Алерты') loadAlerts(alertsLifecycle.consumeReentryReconcile(), undefined, alertsLifecycle.state.snapshot ? 'refresh' : 'initial');
...
function startPendingAlertsReconciliation() { if (activeSection !== 'Алерты') return; if (!alertsLifecycle.consumeReentryReconcile()) return; loadAlerts(true, undefined, alertsLifecycle.state.snapshot ? 'refresh' : 'initial'); }
function finishAlertMutationPresentation(result: { accepted?: boolean; present: boolean; announcement: 'none'|'polite'|'assertive'; message: string; focusKey: string | null }) { if (result.accepted === false) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } if (!result.present) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } render(); if (result.announcement === 'polite') announcePolite(result.message); if (result.announcement === 'assertive') announceAssertive(result.message); if (result.focusKey) requestAnimationFrame(() => focusAlertKey(result.focusKey!)); }
...
function runAlertAction(id: number, action: 'resolve' | 'dismiss') { const previousFocusKey = `alert-${id}-${action}`; const started = alertsLifecycle.startMutation(action, alertsLifecycle.state.routeGeneration, id, previousFocusKey); if (!started.accepted) return; clearFeedbackAnnouncement(); render(); const request = action === 'resolve' ? resolveAlert(id) : dismissAlert(id); request.then((response) => { const result = alertsLifecycle.applyMutationResponse(started.requestId, started.routeGeneration, id, action, response); if (result.accepted === false) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } if (!result.present) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } render(); if (result.announcement === 'polite') announcePolite(result.message); if (result.announcement === 'assertive') announceAssertive(result.message); const focusKey = selectAlertFocusTarget(result.present, previousFocusKey, id, alertFocusCandidates(previousFocusKey, id)); if (focusKey) requestAnimationFrame(() => focusAlertKey(focusKey)); }).catch(() => { const result = alertsLifecycle.finishMutationFailure(started.requestId, started.routeGeneration); if (!result.accepted || !result.present) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } finishAlertMutationPresentation(result); }); }
function runAlertRegeneration() { const started = alertsLifecycle.startMutation('regenerate', alertsLifecycle.state.routeGeneration); if (!started.accepted) return; clearFeedbackAnnouncement(); render(); regenerateAlerts().then((result) => { const mutationResult = alertsLifecycle.finishRegenerationSuccess(started.requestId, started.routeGeneration, result); if (!mutationResult.accepted) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } const filters = alertsLifecycle.displayedFilters(); const refresh = alertsLifecycle.startRead('regeneration-refresh', filters, started.routeGeneration); if (!refresh.accepted) { finishAlertMutationPresentation(mutationResult); return; } return getAlerts(refresh.filters as AlertsState['filters']).then((response) => { const refreshResult = alertsLifecycle.finishReadSuccess(refresh.requestId, refresh.routeGeneration, refresh.filters, response.alerts); finishAlertMutationPresentation({ ...refreshResult, announcement: refreshResult.present ? mutationResult.announcement : 'none', message: refreshResult.present ? mutationResult.message : '', focusKey: refreshResult.present ? selectAlertFocusTarget(true, 'regenerate-alerts', null, [{ key: 'regenerate-alerts', kind: 'action', enabled: true, attached: true }, { key: 'alerts-main-heading', kind: 'main-heading', enabled: true, attached: true }]) : null }); }).catch(() => { const refreshResult = alertsLifecycle.finishReadFailure(refresh.requestId, refresh.routeGeneration, refresh.filters); const message = refreshResult.present ? ALERT_REGENERATION_REFRESH_WARNING_ANNOUNCEMENT : ''; finishAlertMutationPresentation({ ...refreshResult, message, focusKey: refreshResult.present ? 'alerts-main-heading' : null }); }); }).catch(() => { const result = alertsLifecycle.finishMutationFailure(started.requestId, started.routeGeneration); if (!result.accepted || !result.present) { alertsLifecycle.markDetachedMutationSettled(); startPendingAlertsReconciliation(); return; } finishAlertMutationPresentation(result); }); }
