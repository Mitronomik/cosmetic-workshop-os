...
- **New durable contract `docs/audit-log.md`.** It records the accepted `C3-I` product, API, privacy and presentation contract: the `Журнал действий` purpose and its explicit non-purposes; the `actor_type` persistence versus `source` API mapping with no migration and no backfill; exactly one endpoint `GET /api/audit-logs`; the explicit MVP supersession of `GET /api/audit-logs/{id}`; the safe list response (`items`, `total`, `limit`, `offset`, `filter_options`) and the nine-field item; the exclusion of raw `metadata_json`, `entity_id`, table names, stack traces, SQL, paths and payloads; the unknown-code fallbacks `Другое действие` / `Другая сущность` / `Другой источник`; ordering `created_at DESC, id DESC`; pagination `50` / `1` / `200` / `0` / `0`; the seven filters with inclusive `created_from`, exclusive `created_before`, AND combination and the existing structured Russian validation response; append-only read-only behavior; the `/settings/audit-log` route and its full state set; and the complete non-goal list.
...
- **Read-only AuditLog inventory taken from the code, not from documentation.** 50 distinct persisted `action` codes; 19 distinct `entity_type` values; exactly two `actor_type` values, `system` (the `create_log` default) and `user` (only `tax_rate_setting_changed`). The documented source vocabulary `manual` / `import` / `production` / `migration` / `backup` / `restore` is **not persisted anywhere**. `entity_type` `ImportDraft` is PascalCase while every other value is snake_case, and is matched as-is rather than normalized. No call site was edited.
- **Metadata and summary findings.** No persisted metadata contains client notes, allergies, addresses, phones, emails or feedback bodies — it is internal foreign-key IDs, enum codes and counters, which is why `metadata_json` is excluded in full. Persisted summaries embed user-authored values in `client.*` (full name), `client_recipe.*` and `order.*` (titles), catalogue names, and `client_wish.*` (wish title). The wish title is flagged as the one residual sensitive-text case: `C3-I` returns the persisted summary verbatim and never reconstructs it, and narrowing future writes would be a separate, unauthorized write-side decision.
