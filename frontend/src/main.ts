...
function settingsWorkshopProfileCard() {
  ...
  return `<section class="card data-card settings-card settings-profile-card"><h2>Профиль мастерской</h2><p>Эти данные добавляются только в новые сводки мастерской, если профиль заполнен.</p><p class="next-step">Ранее созданные документы не меняются автоматически.</p>${loading ? '<p class="muted-text">Загружаем профиль мастерской…</p>' : ''}${state.error ? `<p class="form-error">${escapeHtml(state.error)}</p>` : ''}${state.message && state.status === 'ready' ? `<p class="success-message">${escapeHtml(state.message)}</p>` : ''}<form class="ingredient-form" data-form="workshop-profile"><div class="form-grid settings-profile-form"><label>Название мастерской<input data-workshop-profile-field="workshop_name" value="${escapeHtml(draft.workshop_name)}" maxlength="120" placeholder="Например, Мастерская Анны" /></label><label>Имя мастера / косметолога<input data-workshop-profile-field="master_name" value="${escapeHtml(draft.master_name)}" maxlength="120" placeholder="Например, Анна Иванова" /></label><label class="full-span">Контактная информация<textarea data-workshop-profile-field="workshop_contact_text" maxlength="500" rows="4" placeholder="Телефон, почта или удобный способ связи">${escapeHtml(draft.workshop_contact_text)}</textarea></label><label class="full-span">Краткое описание / примечание<textarea data-workshop-profile-field="workshop_note" maxlength="500" rows="4" placeholder="Коротко о мастерской для новых сводок">${escapeHtml(draft.workshop_note)}</textarea></label></div><div class="actions"><button class="primary-action" type="submit" ${saving ? 'disabled' : ''}>${saving ? 'Сохраняем…' : 'Сохранить профиль'}</button><button class="secondary-action" type="button" data-action="cancel-workshop-profile" ${saving ? 'disabled' : ''}>Отменить изменения</button></div></form></section>`;
}
