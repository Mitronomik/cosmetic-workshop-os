...
A canonically identical request is a no-op: no upsert, no `updated_at` change, no AuditLog row, the current response shape is returned, and the exact message is `Профиль мастерской уже сохранён без изменений.` Missing-row + empty-request and existing-empty-row + empty-request are no-ops. Configured-to-all-empty is a real change: canonical empty JSON is persisted, the row is retained, and one event is written. This slice adds no delete or Clear behavior.
...
User-facing presentation is backend-owned: `Профиль мастерской изменён`, `Настройка приложения`, `Профиль мастерской обновлён`. Neither summary nor metadata may contain any workshop name, master name, contact text, note, address, phone number, email address, arbitrary profile text, raw JSON, old value or new value. Metadata contains exactly `setting_key`, `changed_fields`, `changed_field_count`, `previous_configured`, and `new_configured`; changed field identifiers are sorted and limited to the four profile fields.

Persistence failure returns HTTP `500` with `code: workshop_profile_not_saved`, message `Не удалось сохранить профиль мастерской. Предыдущие данные сохранены без изменений.` and the safe retry next action documented in `docs/api.md`. Existing validation `422`, database-not-initialized `409`, and successful response shape remain unchanged.
...
- `C3-II-B` — file-backed artifact AuditLog semantics: decided as `CR-009` (ADR 0013) and subdivided. `C3-II-B1` — durable ledger and report-document coverage: `DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #163). `CR-006` is `ACCEPTED AND IMPLEMENTED` (ADR 0014), so `C3-II-B2` is `DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #166); `CR-004` is `ACCEPTED AND IMPLEMENTED` (ADR 0015), so `C3-II-B3` is `DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #167). The C3 artifact-finalization hardening merged as PR #168, so `C3 — COMPLETED — MERGED, EXACT-HEAD VERIFIED AND HARDENED`; contract `docs/audit-log.md` § 17.
- `C4` — Restore и recovery: the product decision is made as `CR-010` (ADR 0016) — launcher-assisted Restore — and **no C4 implementation exists**. `C4-I` is the only authorized runtime slice and is `NOT IMPLEMENTED`. Restore is `NOT IMPLEMENTED`, and this settings contract is unchanged by it: Restore adds no setting, and the pre-restore safety copy uses the existing canonical `before_restore` backup reason rather than a new configurable value.
