...
function orderReadinessPanel(o: Order) {
  if (o.status === 'cancelled' || o.status === 'archived' || !o.is_active) return `<section class="card data-card"><p class="card-kicker">Проверка изготовления</p><h2>Проверка недоступна</h2><p class="next-step">Закрытые, выданные или уже изготовленные заказы нельзя проверять к производству.</p></section>`;
