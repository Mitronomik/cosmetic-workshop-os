...
function submitClientForm(event: SubmitEvent) { event.preventDefault(); const isEdit = Boolean(clientsState.formMode === 'edit' && clientsState.form.id); if (isEdit) syncClientCardDraftFormsFromDom(); const payload = clientPayloadFromForm(event.currentTarget as HTMLFormElement); if (!payload.full_name) { if (isEdit) syncClientCardDraftFormsFromDom(); clientsMessage = ''; clientsError = 'Укажите ФИО клиента, например «Анна Иванова».'; render(); return; } const request = isEdit ? updateClient(clientsState.form.id!, payload) : createClient(payload); request.then((client) => { clientsMessage = isEdit ? 'Карточка клиента обновлена.' : 'Клиент создан.'; clientsError = ''; clientsState.formMode = isEdit ? 'edit' : 'create'; clientsState.showCreateForm = false; clientsState.form = isEdit ? { ...payload, id: client.id } : emptyClientForm(); return getClients(clientsState.includeInactive); }).then((response) => { if (isEdit) syncClientCardDraftFormsFromDom(); clientsState.items = response.clients; clientsStatus = 'ready'; render(); }).catch(() => { if (isEdit) syncClientCardDraftFormsFromDom(); clientsMessage = ''; clientsError = 'Не удалось сохранить клиента. Проверьте ФИО и контактные поля, затем попробуйте еще раз.'; clientsStatus = 'ready'; render(); }); }
