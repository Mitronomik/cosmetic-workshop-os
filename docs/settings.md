...
- canonical API display form may use two decimals, for example `6.00`.
...
Canonical concept: `effective_at`.

- `effective_at` is generated by the backend;
- it is stored and returned as an **ISO-8601 UTC** timestamp;
...
- first configuration, change, and clear each receive a new effective timestamp;
- a no-op save does **not** change `effective_at`.
...
The setting may be explicitly cleared. Preferred future API meaning: `tax_rate_percent: null`.
...
- a no-op clear does not change `effective_at` and creates no `AuditLog`.
...
The setting write and the `AuditLog` write must be **atomic**. If `AuditLog` creation fails, the tax-setting update must roll back and no partial setting change may remain.
...
Verified read-only against `origin/main` at `09d11fc32db6ae57f99d522c4aa71e223e4e01a5`. These are implementation constraints for `C1-I`, not authorizations to change behavior in this decision PR.
...
- **`app_settings.updated_at` is not ISO-8601.** The column defaults to SQLite `CURRENT_TIMESTAMP`, which produces `YYYY-MM-DD HH:MM:SS` in UTC without a `T` separator or offset. `effective_at` must be normalized to ISO-8601 UTC at the service boundary. Do not change the column type or the existing default for this slice.
