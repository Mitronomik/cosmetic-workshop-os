...
import { createStockMovementLotDetailLifecycle, mutationDisabled, mutationReadonly, restoreMutationGuards, packagingPageMutationActiveState, type StockMovementLotDetailRequest } from './mutation-lifecycle.js';
...
const recipeVersionTopFieldLabels: Record<string, string> = {
  ...
  created_from_version_id: 'Исходная версия',
  ingredients: 'Состав',
};
...
function reindexRecipeVersionValidation(state: FormValidationState, removedIndex: number): FormValidationState { const fieldErrors: Record<string, string[]> = {}; for (const [field, messages] of Object.entries(state.fieldErrors)) { const match = /^ingredients\.(\d+)\.(.+)$/.exec(field); if (!match) { fieldErrors[field] = messages; continue; } const index = Number(match[1]); if (index === removedIndex) continue; const nextIndex = index > removedIndex ? index - 1 : index; fieldErrors[`ingredients.${nextIndex}.${match[2]}`] = messages; } return { ...state, fieldErrors }; }
