...
import { canOpenOrderProductionConfirmation, clearOrderSourceValidation, createOrderMutationController, emptyOrderValidationProvenance, orderOperationError, orderOperationErrorFor, orderPayloadFromDraft, orderProductionIsClosed, orderRequestOwnerMatches, ownerFromOrderRequest, type OrderContextSnapshot, type OrderOperationError, type OrderRequestSnapshot, type OrderTransientRequestOwner, type OrderValidationProvenance } from './order-mutation-lifecycle.js';
...
function orderReadinessPanel(o: Order) {
  ...
  if (readinessError) return `<section class="card error-card"><p class="card-kicker">Проверка изготовления</p><h2>Не удалось проверить готовность производства.</h2><p>${escapeHtml(readinessError)}</p><p class="next-step">Проверьте, что локальное приложение запущено, и повторите проверку.</p><button class="primary-action" type="button" data-action="retry-order-readiness" data-id="${o.id}">Повторить проверку</button></section>`;
...
function checkOrderReadiness(id:number){ if (orderFormMutationActive()) return; const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; if(!order || orderProductionClosed(order)) return; const snapshot=orderMutationController.beginRequest('readiness', currentOrderContext(), { requestedOrderId: id }); ordersState.readinessRequestOwner=ownerFromOrderRequest(snapshot, id, 'readiness'); ordersState.readinessLoadingOrderId=id; ordersState.readinessError=''; ordersState.readinessErrorInfo=null; render(); checkOrderProductionReadiness(id).then((result)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !readinessOwnerMatches(snapshot, id)) return; ordersState.readinessByOrderId[id]=result; ordersState.readinessLoadingOrderId=null; ordersState.readinessRequestOwner=null; ordersState.readinessError=''; ordersState.readinessErrorInfo=null; render(); }).catch((e)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id || !readinessOwnerMatches(snapshot, id)) return; ordersState.readinessLoadingOrderId=null; ordersState.readinessRequestOwner=null; const msg=e instanceof Error && e.message && e.message !== 'API request failed' ? e.message : ''; ordersState.readinessError=msg || 'Не удалось проверить готовность производства.'; ordersState.readinessErrorInfo=orderOperationError(id, ordersState.readinessError); render(); }); }
function openProductionConfirmation(id:number){ const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=ordersState.readinessByOrderId[id]; if(!canOpenOrderProductionConfirmation(orderFormMutationActive(), order, readiness)) return; ordersState.productionConfirmingOrderId=id; ordersState.productionError=''; ordersState.productionErrorInfo=null; if(!(id in ordersState.productionNotesByOrderId)) ordersState.productionNotesByOrderId[id]=''; render(); }
