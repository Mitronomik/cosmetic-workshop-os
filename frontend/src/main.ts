...
function moveClientRecipeCompositionLine(index: number, direction: number) { saveClientRecipeCompositionDraftFromDom(); const next = index + direction; const draft = clientRecipesState.compositionEditor.draft; if (next < 0 || next >= draft.length) return; [draft[index], draft[next]] = [draft[next], draft[index]]; render(); }
