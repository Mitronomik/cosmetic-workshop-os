import { clearFieldValidation, clearIndexedCollectionValidation, emptyFormValidationState, hasFormValidationErrors, normalizeBackendValidation, type FormValidationState } from './form-validation.js';
...
function bindEvents(root: HTMLElement) {
  ...
  root.querySelector<HTMLButtonElement>('[data-action="close-order-form"]')?.addEventListener('click', () => { if (orderFormMutationActive()) return; ordersState.showForm = false; ordersState.form = emptyOrderForm(); orderValidation = emptyFormValidationState(); render(); });
  root.querySelector<HTMLSelectElement>('[data-form="order"] select[name="client_id"]')?.addEventListener('change', () => { if (orderFormMutationActive()) return; saveOrderFormDraftFromDom(); ordersState.form.client_recipe_id = ''; orderValidation = clearFieldValidation(clearFieldValidation(orderValidation, 'client_id'), 'client_recipe_id'); render(); });
  root.querySelector<HTMLButtonElement>('[data-action="close-order-detail"]')?.addEventListener('click', () => { if (orderFormMutationActive()) return; ordersState.selectedOrder = null; render(); });
...
function normalizeOrderValidation(payload: unknown, sourceType: OrderSourceType): FormValidationState {
  ...
  for (const [field, messages] of Object.entries(raw.fieldErrors)) {
    ...
    else if (field === 'recipe_version_id' && sourceType !== 'recipe_version') formErrors.push(...messages);
    else if (field === 'client_recipe_id' && sourceType !== 'client_recipe') formErrors.push(...messages);
...
function orderRenderSafeAfterRead() { return !orderFormMutationActive() && !(ordersState.showForm && hasFormValidationErrors(orderValidation)); }
function loadOrders(force=false){ if (orderFormMutationActive()) return; if(!force && ordersStatus==='loading') return; if(!force && ordersStatus==='ready') return; ordersStatus='loading'; ordersError=''; render(); Promise.all([getOrders(true), loadOrderReferenceData()]).then(([orders])=>{ ordersState.items=orders.orders; ordersStatus='ready'; if (orderRenderSafeAfterRead()) render(); }).catch(()=>{ ordersStatus='error'; ordersError='Не удалось загрузить заказы. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead()) render(); }); }
...
function reloadOrderReferencesForForm(){ if (orderFormMutationActive()) return; if (ordersState.referenceLoading) return; ordersState.referenceLoading=true; ordersState.referenceError=''; render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; ordersStatus='ready'; if (orderRenderSafeAfterRead()) render(); focusOrderFormClient(); }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead()) render(); }); }
function openOrderCreate(){ if (orderFormMutationActive()) return; orderValidation = emptyFormValidationState(); orderRefreshWarning = ''; if (ordersState.referenceLoading && ordersState.showForm && ordersState.formMode==='create') return; ordersState.formMode='create'; ordersState.form=emptyOrderForm(); ordersState.showForm=true; ordersState.selectedOrder=null; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; ordersStatus='ready'; if (orderRenderSafeAfterRead()) render(); focusOrderFormClient(); }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; render(); }); }
function openOrder(id:number){ if (orderFormMutationActive()) return; const fallback=ordersState.items.find(i=>i.id===id) ?? null; ordersState.selectedOrder=fallback; ordersState.showForm=false; ordersMessage=''; ordersError=''; render(); getOrder(id).then((order)=>{ ordersState.selectedOrder=order; render(); }).catch(()=>{ ordersError='Не удалось открыть карточку заказа. Попробуйте обновить список.'; render(); }); }
...
function editOrder(id:number){ if (orderFormMutationActive()) return; orderValidation = emptyFormValidationState(); orderRefreshWarning = ''; const o=ordersState.items.find(i=>i.id===id); if(!o) return; ordersState.selectedOrder=o; ordersState.formMode='edit'; ordersState.form=formFromOrder(o); ordersState.showForm=true; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; render(); focusOrderFormClient(); }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; render(); }); }
function submitOrderForm(event: SubmitEvent){ event.preventDefault(); if (orderSubmitting) return; const form=event.currentTarget as HTMLFormElement; const payload=orderPayloadFromForm(form); if (typeof payload==='string') { ordersError=payload; return; } saveOrderFormDraftFromDom(); const mode=ordersState.formMode; const editedId=ordersState.form.id; const sourceType=ordersState.form.source_type; const token=++orderSubmitToken; orderSubmitting=true; orderValidation=emptyFormValidationState(); orderRefreshWarning=''; ordersError=''; applyValidationToOrderForm(orderValidation); disableOrderMutationControls(); const request=mode==='edit' && editedId ? updateOrder(editedId, payload) : createOrder(payload); request.then((saved)=>{ if (token !== orderSubmitToken) return Promise.reject({ stale:true }); orderValidation=emptyFormValidationState(); ordersMessage=mode==='edit'?'Заказ сохранён.':'Заказ создан.'; ordersError=''; ordersState.items = [saved, ...ordersState.items.filter((item)=>item.id!==saved.id)]; ordersState.selectedOrder=saved; ordersState.showForm=false; orderSubmitting=false; render(); return getOrders(true).then((response)=>{ if (token !== orderSubmitToken) return; ordersState.items=response.orders; orderRefreshWarning=''; render(); }).catch(()=>{ if (token !== orderSubmitToken) return; orderRefreshWarning='Заказ сохранён, но список не удалось обновить автоматически.'; render(); }); }).catch((e)=>{ if ((e as {stale?: boolean})?.stale || token !== orderSubmitToken) return; orderSubmitting=false; ordersMessage=''; orderRefreshWarning=''; ordersState.formMode=mode; ordersState.form.id=editedId; ordersState.showForm=true; ordersError=''; orderValidation=normalizeOrderValidation(apiValidationPayload(e), sourceType); applyValidationToOrderForm(orderValidation); restoreOrderMutationControls(); announceAssertive('Проверьте заказ. Ошибки показаны рядом с полями.'); }); }
