...
- **Persistence mapping.** The database column stays `actor_type`; the durable domain and API name is `source`. `C3-I` maps one to the other at read time. No column rename, no migration, no backfill, and no change to existing write call sites merely to rename the field.
...
- **Safe read model.** The response carries `items`, `total`, `limit`, `offset`, `filter_options`; each item carries only `id`, `created_at`, `action`, `action_label`, `entity_type`, `entity_label`, `summary`, `source`, `source_label`. Raw `metadata_json` is never returned, and neither are table names, stack traces, SQL, filesystem paths or raw payloads.
- **Ordering and pagination.** `created_at DESC, id DESC`; `limit` default `50`, minimum `1`, maximum `200`; `offset` default `0`, minimum `0`. No unbounded history.
- **Filters.** `created_from` (inclusive), `created_before` (exclusive), `action`, `entity_type`, `source`, `limit`, `offset`, combined with logical AND, with ISO-8601 UTC timestamps, the existing structured Russian validation response for malformed input, rejection of `created_before <= created_from`, filter options derived from actually persisted values, and no writes.
- **Read-only.** Reads write no AuditLog record, mutate no business table, create no file, change no setting, trigger no regeneration, and perform no cleanup or normalization of historical rows. AuditLog remains append-only.
- **Frontend.** Canonical route `/settings/audit-log`, title `Журнал действий`, with the full state set — loading, empty, filtered-empty, refresh failure retaining the previously accepted list, initial-load failure, narrow viewport and keyboard accessibility. No raw codes, no JSON, no `metadata_json`, no table names, no internal entity IDs, no stack traces, no SQL, no developer paths and no GitHub or PR terminology. Focused modules only; `frontend/src/main.ts` must not grow net.

### C3-I non-goals

AuditLog edit; AuditLog delete; rollback or undo; restore from AuditLog; detail endpoint; metadata viewer; raw JSON viewer; CSV/XLSX/PDF audit export; charts; analytics; search over sensitive text; roles or permissions; multi-user actors; remote audit shipping; cloud sync; retention policies; log compaction; schema migration; backfill; changes to existing write semantics; C4; Restore; packaging; update flow; release-candidate smoke.
