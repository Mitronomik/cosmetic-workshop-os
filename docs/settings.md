...
- `C3-II-B` — file-backed artifact AuditLog semantics: decided as `CR-009` (ADR 0013) and subdivided. `C3-II-B1` — durable ledger and report-document coverage: `DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #163). `CR-006` is now `ACCEPTED` (ADR 0014), so `C3-II-B2` is `AUTHORIZED AFTER THE CR-006 DECISION PR MERGES — NOT IMPLEMENTED`; `C3-II-B3` remains `BLOCKED BY CR-004 — NOT AUTHORIZED`; contract `docs/audit-log.md` § 17.
