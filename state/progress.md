...
> **HISTORICAL PRE-MERGE RECORD — SUPERSEDED.** This entry described `C2-III-A` while it was still on an unmerged PR branch, and its slice statuses were true when written. `C2-III-A` merged as PR #154 and is `DONE — MERGED AND EXACT-HEAD VERIFIED`; `C2-III-B` is no longer `PLANNED — BLOCKED` with no PR number — it is `IMPLEMENTED ON PR BRANCH — NOT MERGED` on PR #157, branch `codex/c2-iii-b-snapshot-backed-reports`.
