...
import { clearOrderSourceValidation, createOrderMutationController, emptyOrderValidationProvenance, orderPayloadFromDraft, type OrderContextSnapshot, type OrderRequestSnapshot, type OrderValidationProvenance } from './order-mutation-lifecycle.js';
...
function openProductionBatchByOrder(id:number) { if (orderFormMutationActive()) return; const snapshot=orderMutationController.beginRequest('production', currentOrderContext(), { requestedOrderId: id }); ordersState.productionLoadingOrderId=id; ordersState.productionError=''; render(); getProductionBatchByOrder(id).then((batch)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id) return; ordersState.productionByOrderId[id]=batch; ordersState.productionLoadingOrderId=null; productionHistoryState.selectedBatch=batch; productionHistoryState.detailStatus='ready'; productionHistoryState.detailError=''; activeSection='Производство'; window.history.pushState({}, '', pathForSection(activeSection)); loadProductionHistory(true); render(); }).catch((e)=>{ if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id) return; ordersState.productionLoadingOrderId=null; const status=typeof e==='object' && e && 'status' in e ? Number((e as {status?: number}).status) : 0; ordersState.productionError=status===404 ? 'Для этого заказа производственная партия пока не найдена.' : 'Не удалось открыть производственную партию. Обновите историю производства и попробуйте ещё раз.'; render(); }); }
...
function openOrderCreate(){ if (orderFormMutationActive()) return; if (ordersState.referenceLoading && ordersState.showForm && ordersState.formMode==='create') return; bumpOrderContext(); orderValidation = emptyFormValidationState(); orderValidationProvenance = emptyOrderValidationProvenance(); orderRefreshWarning = ''; ordersState.formMode='create'; ordersState.form=emptyOrderForm(); ordersState.showForm=true; ordersState.selectedOrder=null; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; beginOrderReferenceLoad(true); }
