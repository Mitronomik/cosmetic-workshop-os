...
import { clearOrderSourceValidation, emptyOrderValidationProvenance, orderPayloadFromDraft, orderReadCanRender, type OrderContextSnapshot, type OrderValidationProvenance } from './order-mutation-lifecycle.js';
...
let orderSubmitToken = 0;
let orderRefreshWarning = '';
let orderValidationProvenance: OrderValidationProvenance = emptyOrderValidationProvenance();
let orderContextToken = 0;
let orderTargetedValidationToken = 0;
function bumpOrderContext() { orderContextToken += 1; return orderContextToken; }
function currentOrderContext(): OrderContextSnapshot { return { contextToken: orderContextToken, targetedValidationToken: orderTargetedValidationToken, formMode: ordersState.formMode, editedOrderId: ordersState.form.id, selectedOrderId: ordersState.selectedOrder?.id ?? null, showForm: ordersState.showForm }; }
function orderReadCanRenderCaptured(snapshot: OrderContextSnapshot) { return orderReadCanRender(snapshot, currentOrderContext()); }
...
function bindEvents(root: HTMLElement) {
  ...
  root.querySelector<HTMLButtonElement>('[data-action="reload-orders"]')?.addEventListener('click', () => { if (!orderFormMutationActive()) loadOrders(true); });
  ...
  root.querySelector<HTMLButtonElement>('[data-action="close-order-form"]')?.addEventListener('click', () => { if (orderFormMutationActive()) return; bumpOrderContext(); ordersState.showForm = false; ordersState.form = emptyOrderForm(); orderValidation = emptyFormValidationState(); orderValidationProvenance = emptyOrderValidationProvenance(); render(); });
  ...
  root.querySelector<HTMLButtonElement>('[data-action="close-order-detail"]')?.addEventListener('click', () => { if (orderFormMutationActive()) return; bumpOrderContext(); ordersState.selectedOrder = null; render(); });
...
function orderFormMutationActive() { return orderSubmitting; }
...
function loadOrders(force=false){ if (orderFormMutationActive()) return; if(!force && ordersStatus==='loading') return; if(!force && ordersStatus==='ready') return; const snapshot=currentOrderContext(); ordersStatus='loading'; ordersError=''; render(); Promise.all([getOrders(true), loadOrderReferenceData()]).then(([orders])=>{ ordersState.items=orders.orders; ordersStatus='ready'; orderRefreshWarning=''; if (orderRenderSafeAfterRead(snapshot)) render(); }).catch(()=>{ ordersStatus='error'; ordersError='Не удалось загрузить заказы. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead(snapshot)) render(); }); }
...
function reloadOrderReferencesForForm(){ if (orderFormMutationActive()) return; if (ordersState.referenceLoading) return; const snapshot=currentOrderContext(); ordersState.referenceLoading=true; ordersState.referenceError=''; render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; ordersStatus='ready'; if (orderRenderSafeAfterRead(snapshot)) { render(); focusOrderFormClient(); } }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead(snapshot)) render(); }); }
function openOrderCreate(){ if (orderFormMutationActive()) return; bumpOrderContext(); orderValidation = emptyFormValidationState(); orderValidationProvenance = emptyOrderValidationProvenance(); orderRefreshWarning = ''; if (ordersState.referenceLoading && ordersState.showForm && ordersState.formMode==='create') return; ordersState.formMode='create'; ordersState.form=emptyOrderForm(); ordersState.showForm=true; ordersState.selectedOrder=null; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; const snapshot=currentOrderContext(); render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; ordersStatus='ready'; if (orderRenderSafeAfterRead(snapshot)) { render(); focusOrderFormClient(); } }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead(snapshot)) render(); }); }
function openOrder(id:number){ if (orderFormMutationActive()) return; bumpOrderContext(); const fallback=ordersState.items.find(i=>i.id===id) ?? null; ordersState.selectedOrder=fallback; ordersState.showForm=false; ordersMessage=''; ordersError=''; const snapshot=currentOrderContext(); render(); getOrder(id).then((order)=>{ if (!orderRenderSafeAfterRead(snapshot) || ordersState.selectedOrder?.id !== id) return; ordersState.selectedOrder=order; render(); }).catch(()=>{ if (!orderRenderSafeAfterRead(snapshot)) return; ordersError='Не удалось открыть карточку заказа. Попробуйте обновить список.'; render(); }); }
function checkOrderReadiness(id:number){ if (orderFormMutationActive()) return; const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; if(!order || orderProductionClosed(order)) return; const snapshot=currentOrderContext(); ordersState.readinessLoadingOrderId=id; ordersState.readinessError=''; render(); checkOrderProductionReadiness(id).then((result)=>{ ordersState.readinessByOrderId[id]=result; ordersState.readinessLoadingOrderId=null; ordersState.readinessError=''; if (orderRenderSafeAfterRead(snapshot)) render(); }).catch((e)=>{ ordersState.readinessLoadingOrderId=null; const msg=e instanceof Error && e.message && e.message !== 'API request failed' ? e.message : ''; ordersState.readinessError=msg || 'Не удалось проверить готовность производства.'; if (orderRenderSafeAfterRead(snapshot)) render(); }); }

function openProductionConfirmation(id:number){ if (orderFormMutationActive()) return; const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=ordersState.readinessByOrderId[id]; if(!order || orderProductionClosed(order) || !readiness?.can_produce) return; ordersState.productionConfirmingOrderId=id; ordersState.productionError=''; render(); }
function cancelProductionConfirmation(id:number){ if (orderFormMutationActive()) return; if(ordersState.productionLoadingOrderId===id) return; if(ordersState.productionConfirmingOrderId===id) ordersState.productionConfirmingOrderId=null; render(); }
function confirmProduction(id:number){ if (orderFormMutationActive()) return; const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=ordersState.readinessByOrderId[id]; if(!order || orderProductionClosed(order) || !readiness?.can_produce) return; const snapshot=currentOrderContext(); ordersState.productionLoadingOrderId=id; ordersState.productionError=''; render(); produceOrder(id, ordersState.productionNotesByOrderId[id]).then((batch)=>{ ordersState.productionByOrderId[id]=batch; delete ordersState.readinessByOrderId[id]; ordersState.productionLoadingOrderId=null; ordersState.productionConfirmingOrderId=null; ordersState.productionError=''; ordersState.productionNotesByOrderId[id]=''; return Promise.all([getOrders(true), getOrder(id)]); }).then(([orders, updated])=>{ ordersState.items=orders.orders; if (orderRenderSafeAfterRead(snapshot)) { ordersState.selectedOrder=updated; render(); } }).catch((e)=>{ ordersState.productionLoadingOrderId=null; ordersState.productionError=humanProductionError(e); if (orderRenderSafeAfterRead(snapshot)) render(); }); }

function editOrder(id:number){ if (orderFormMutationActive()) return; const o=ordersState.items.find(i=>i.id===id); if(!o) return; bumpOrderContext(); orderValidation = emptyFormValidationState(); orderValidationProvenance = emptyOrderValidationProvenance(); orderRefreshWarning = ''; ordersState.selectedOrder=o; ordersState.formMode='edit'; ordersState.form=formFromOrder(o); ordersState.showForm=true; ordersMessage=''; ordersError=''; ordersState.referenceLoading=true; ordersState.referenceError=''; const snapshot=currentOrderContext(); render(); loadOrderReferenceData().then(()=>{ ordersState.referenceLoading=false; ordersState.referenceError=''; if (orderRenderSafeAfterRead(snapshot)) { render(); focusOrderFormClient(); } }).catch(()=>{ ordersState.referenceLoading=false; ordersState.referenceError='Не удалось загрузить клиентов и рецепты для заказа. Проверьте локальное приложение и попробуйте ещё раз.'; if (orderRenderSafeAfterRead(snapshot)) render(); }); }
function submitOrderForm(event: SubmitEvent){ event.preventDefault(); if (orderSubmitting) return; const form=event.currentTarget as HTMLFormElement; const payload=orderPayloadFromForm(form); saveOrderFormDraftFromDom(); const mode=ordersState.formMode; const editedId=ordersState.form.id; const sourceType=ordersState.form.source_type; const submitContext=currentOrderContext(); const token=++orderSubmitToken; orderSubmitting=true; orderValidation=emptyFormValidationState(); orderValidationProvenance=emptyOrderValidationProvenance(); orderRefreshWarning=''; ordersError=''; applyValidationToOrderForm(orderValidation); disableOrderMutationControls(); const request=mode==='edit' && editedId ? updateOrder(editedId, payload) : createOrder(payload); request.then((saved)=>{ if (token !== orderSubmitToken || !orderReadCanRender(submitContext, { ...currentOrderContext(), targetedValidationToken: submitContext.targetedValidationToken })) return Promise.reject({ stale:true }); orderValidation=emptyFormValidationState(); orderValidationProvenance=emptyOrderValidationProvenance(); ordersMessage=mode==='edit'?'Заказ сохранён.':'Заказ создан.'; ordersError=''; ordersState.items = [saved, ...ordersState.items.filter((item)=>item.id!==saved.id)]; ordersState.selectedOrder=saved; ordersState.showForm=false; orderSubmitting=false; bumpOrderContext(); render(); const refreshContext=currentOrderContext(); return getOrders(true).then((response)=>{ ordersState.items=response.orders; orderRefreshWarning=''; if (orderRenderSafeAfterRead(refreshContext)) render(); }).catch(()=>{ orderRefreshWarning='Заказ сохранён, но список не удалось обновить автоматически.'; if (orderRenderSafeAfterRead(refreshContext)) render(); }); }).catch((e)=>{ if ((e as {stale?: boolean})?.stale || token !== orderSubmitToken) return; orderSubmitting=false; ordersMessage=''; orderRefreshWarning=''; ordersState.formMode=mode; ordersState.form.id=editedId; ordersState.showForm=true; ordersError=''; const normalized=normalizeOrderValidation(apiValidationPayload(e), sourceType); orderValidation=normalized.validation; orderValidationProvenance=normalized.provenance; orderTargetedValidationToken += 1; applyValidationToOrderForm(orderValidation); restoreOrderMutationControls(); announceAssertive('Проверьте заказ. Ошибки показаны рядом с полями.'); }); }
function cancelOrder(id:number){ if (orderFormMutationActive()) return; if(!window.confirm('Отменить заказ? Заказ останется в истории, склад и производство не будут затронуты.')) return; cancelOrderRequest(id).then((saved)=>{ ordersMessage='Заказ отменён.'; ordersError=''; ordersState.selectedOrder=saved; return getOrders(true); }).then(r=>{ ordersState.items=r.orders; render(); }).catch((e)=>{ ordersError=humanOrderError(e); render(); }); }
function archiveOrder(id:number){ if (orderFormMutationActive()) return; if(!window.confirm('Архивировать заказ? Заказ не будет удалён и останется в истории.')) return; archiveOrderRequest(id).then((saved)=>{ ordersMessage='Заказ архивирован.'; ordersError=''; ordersState.selectedOrder=saved; return getOrders(true); }).then(r=>{ ordersState.items=r.orders; render(); }).catch((e)=>{ ordersError=humanOrderError(e); render(); }); }
