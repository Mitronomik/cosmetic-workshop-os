...
- **Next slice — exactly one active:** `R3 — Repair purchase-suggestions API smoke seeding`, test-only. It changes only the invalid `lot_qty="0"` seed at `backend/app/tests/test_purchase_suggestions.py:214`, using a positive lot quantity and a higher minimum-stock threshold, and preserves and executes every existing API and no-mutation assertion. No production-code change, no skip, xfail, deletion, rename, or weakened assertion. Required smoke is the **backend suite only**; the slice changes no runtime surface, so no browser, visual, or route-rendering check applies.
