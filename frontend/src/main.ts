...
function backupHistoryItem(backup: BackupFileResponse) { return `<article class="recipe-line backup-item"><div class="section-heading"><div><h3>${escapeHtml(backup.filename)}</h3><p><span class="pill info">${backupReasonLabel(backup.reason)}</span> <span class="pill muted">${formatFileSize(backup.size_bytes)}</span></p></div><small>${formatDateTime(backup.created_at || '')}</small></div><p><strong>Локальный путь:</strong><br><code class="path-text">${escapeHtml(backup.path)}</code></p><p class="next-step">Только файл копии. Восстановление будет добавлено отдельным шагом.</p></article>`; }
function backupNonGoalsCard() { return `<section class="card data-card"><p class="card-kicker">Честные границы</p><h2>Пока не реализовано</h2><p>Сейчас этот экран только создаёт и показывает локальные резервные копии. Восстановление и импорт будут добавлены отдельными PR.</p><ul class="checklist compact-list"><li>Восстановление из копии</li><li>Скачивание копии из браузера</li><li>Автоматические копии по расписанию</li><li>Cloud backup</li><li>Export CSV/XLSX</li><li>Import CSV/XLSX</li></ul></section>`; }
...
function reportDocumentsBoundariesCard() { return `<section class="card data-card"><p class="card-kicker">Честные границы</p><h2>Что важно знать</h2><ul class="checklist compact-list"><li>Markdown остается доступен; PDF создается только явным действием, если backend сообщает поддержку.</li><li>DOCX не создается в этом PR.</li><li>Документ строится из backend-отчетов и не пересчитывает данные во frontend.</li><li>Создание документа не меняет заказы, склад, производство или закупки.</li><li>Это не бухгалтерский и не налоговый документ.</li><li>Налоговые ставки не придумываются.</li></ul></section>`; }
...
function settingsPage() {
  return `<div class="settings-layout"><section class="card data-card settings-hero"><h2>Настройки</h2><p>Здесь можно сохранить данные мастерской для новых сводок и перейти к безопасной работе с локальными файлами.</p></section>${settingsWorkshopProfileCard()}${settingsLocalDataSection()}</div>`;
}
...
function loadReports(force = false) {
  ...
  Promise.all([getReportsOverview(), getInventoryReport(), getOrdersReport(), getProductionReport(), getFinanceReport()])
    .then(([overview, inventory, orders, production, finance]) => { reportsUiState = { ...reportsUiState, status: 'ready', error: '', message: force ? 'Отчеты обновлены. Данные только прочитаны из локального backend.' : '', overview, inventory, orders, production, finance }; render(); })
...
function cancelWorkshopProfileChanges() { const profile = workshopProfileUiState.profile; if (!isWorkshopProfileFormAvailable() || profile === null) return; workshopProfileUiState.draft = { ...profile }; workshopProfileUiState.error = ''; workshopProfileUiState.message = ''; render(); }
function submitWorkshopProfileForm(event: Event) { event.preventDefault(); if (!isWorkshopProfileFormAvailable()) return; workshopProfileUiState.actionStatus = 'saving'; workshopProfileUiState.error = ''; workshopProfileUiState.message = ''; render(); updateWorkshopProfile(workshopProfileUiState.draft).then((data) => { workshopProfileUiState = { status: 'ready', actionStatus: 'idle', profile: data.profile, draft: { ...data.profile }, error: '', message: 'Профиль мастерской сохранен.' }; render(); }).catch((error: Error) => { workshopProfileUiState.actionStatus = 'idle'; workshopProfileUiState.error = error.message || 'Не удалось сохранить профиль мастерской. Данные рецептов, склада и заказов не изменялись.'; render(); }); }
