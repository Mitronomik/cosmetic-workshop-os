...
function disableStockMovementSubmitControls() { const form = document.querySelector<HTMLFormElement>('[data-form="stock-movement"]'); if (form) { form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input:not([readonly]), textarea').forEach((el) => { el.readOnly = true; el.dataset.validationReadonly = 'true'; }); form.querySelectorAll<HTMLSelectElement>('select').forEach((el) => el.setAttribute('disabled', '')); form.querySelector<HTMLButtonElement>('button[type="submit"]')?.setAttribute('disabled', ''); const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]'); if (submit) submit.textContent = 'Создаём…'; } document.querySelector<HTMLSelectElement>('[data-action="select-stock-lot"]')?.setAttribute('disabled', ''); document.querySelector<HTMLButtonElement>('[data-action="reload-stock-movements"]')?.setAttribute('disabled', ''); }
function disablePackagingItemSubmitControls() { const form = document.querySelector<HTMLFormElement>('[data-form="packaging-item"]'); if (form) { form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input, textarea').forEach((el) => { el.readOnly = true; el.dataset.validationReadonly = 'true'; }); form.querySelectorAll<HTMLSelectElement>('select').forEach((el) => el.setAttribute('disabled', '')); form.querySelectorAll('button').forEach((b) => b.setAttribute('disabled', '')); const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]'); if (submit) submit.textContent = 'Сохраняем…'; } document.querySelectorAll<HTMLButtonElement>('[data-action="new-packaging-item"], [data-action="edit-packaging-item"], [data-action="deactivate-packaging-item"], [data-action="reload-packaging-items"]').forEach((b) => b.setAttribute('disabled', '')); }
function reenableStockMovementSubmitButtons() { const form = document.querySelector<HTMLFormElement>('[data-form="stock-movement"]'); if (form) { form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input, textarea').forEach((el) => { if (el.dataset.validationReadonly === 'true') { el.readOnly = false; delete el.dataset.validationReadonly; } }); form.querySelectorAll<HTMLSelectElement>('select').forEach((el) => el.removeAttribute('disabled')); form.querySelectorAll('button').forEach((b) => b.removeAttribute('disabled')); const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]'); if (submit) submit.textContent = 'Создать движение'; } document.querySelector<HTMLSelectElement>('[data-action="select-stock-lot"]')?.removeAttribute('disabled'); document.querySelector<HTMLButtonElement>('[data-action="reload-stock-movements"]')?.removeAttribute('disabled'); }
function reenablePackagingItemSubmitButtons() { const form = document.querySelector<HTMLFormElement>('[data-form="packaging-item"]'); if (form) { form.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('input, textarea').forEach((el) => { if (el.dataset.validationReadonly === 'true') { el.readOnly = false; delete el.dataset.validationReadonly; } }); form.querySelectorAll<HTMLSelectElement>('select').forEach((el) => el.removeAttribute('disabled')); form.querySelectorAll('button').forEach((b) => b.removeAttribute('disabled')); const submit = form.querySelector<HTMLButtonElement>('button[type="submit"]'); if (submit) submit.textContent = packagingItemsState.formMode === 'edit' ? 'Сохранить изменения' : 'Создать тару'; } document.querySelectorAll<HTMLButtonElement>('[data-action="new-packaging-item"], [data-action="edit-packaging-item"], [data-action="deactivate-packaging-item"], [data-action="reload-packaging-items"]').forEach((b) => b.removeAttribute('disabled')); }
