...
## Tests status
Python syntax compilation passed. Scoped domain primitive tests passed, including fractional-count rejection. Frontend build and `make build` passed. Full backend pytest and `make test` are blocked by the environment because FastAPI is not installed; this is not caused by PR5 domain primitive assertions.
