...
`R3 — Repair purchase-suggestions API smoke seeding` is the only active slice, and it is **test-only**. It changes exactly one value: `lot_qty="0"` becomes `lot_qty="1"` in the `seed_ready(...)` call at `backend/app/tests/test_purchase_suggestions.py:214`. The existing `minimum_stock='10'` setup on the following lines stays unchanged, because that threshold is already higher than the new lot quantity of `1`, so the below-minimum condition the test needs remains true. No other line in the test may change. The test then reaches the `/api/purchase-suggestions` HTTP surface and its existing assertions — including the three no-mutation assertions — actually execute. No production code changes, and its required smoke is the backend suite only. `R2 — Import draft issue-count contract alignment` is the next deferred slice. Contract: `state/current-focus.md`. Evidence: `docs/backend-baseline-failure-triage.md`.
