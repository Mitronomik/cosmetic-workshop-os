...
function formatFileSize(bytes: number | null | undefined): string { return artifactSize(bytes); }
...
function settingsLocalDataSection() {
  ...
  return `<section class="card data-card settings-card"><h2>Локальные данные</h2><p>Данные «Мастерской косметолога» хранятся отдельно от файлов приложения и остаются на этом компьютере. Приложение работает local-first: ежедневная работа не требует облака или обязательного интернета.</p><p class="next-step">Перед обновлением приложения, переносом данных или крупной миграцией сначала создайте резервную копию.</p><div class="settings-data-path"><span>${escapeHtml(artifactFolderLabel('data'))}</span><strong>${local?.user_data_separate_from_code ? 'Данные отделены от приложения' : 'Проверьте расположение данных'}</strong><p>Ищите рабочие файлы в папке данных приложения для «Мастерской косметолога».</p></div>${local?.user_data_path_display ? `<details class="technical-details settings-technical-details"><summary>Путь к папке данных</summary><code class="path-text">${escapeHtml(local.user_data_path_display)}</code></details>` : '<p class="muted-text">Путь к папке данных будет показан, когда локальное приложение вернёт эти сведения.</p>'}${local?.backup_before_migration_required ? feedbackMessage('warning', 'Перед миграцией или обновлением обязательно создайте резервную копию.') : ''}${settingsLocalDataActions()}</section>`;
}
