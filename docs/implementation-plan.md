...
Статус: **активировать после проверки и merge PR #106**
...
- runtime-функциональность реализована через PR98;
...
- PR106 вводит shared feedback и announcement semantics для `/settings`, `/exports`, `/report-documents`, `/imports`, `/demo-data`;
- PR106 нельзя мержить без локального Hermes smoke для Import Apply, refresh failure, structured mutation failure, Settings, responsive, keyboard и announcers;
- полный аудит Hermes выполнен на `main` после PR105, поэтому feedback- и demo-related findings нужно повторно проверять после PR106;
- новые runtime-изменения нельзя начинать, пока PR106 не закрыт и не зафиксирован в state-документах.
...
### Перепроверяем после PR106

- одновременные success и error на `/demo-data`;
- duplicate/stale feedback на маршрутах PR106;
- incorrect polite/assertive announcements.
...
Завершить shared feedback semantics без добавления новых audit fixes в текущую ветку.

## Обязательная проверка

- обычный Import Apply отправляет ровно один POST и создаёт одну запись;
- Apply success + failed refresh показывает видимый warning и сохраняет success;
- отсутствует `Не удалось применить черновик` после успешной mutation;
- structured Apply conflict показывает row/field/code/message;
- partial writes и дубли отсутствуют;
- Settings save/edit/cancel работают корректно;
- polite/assertive announcers срабатывают один раз на действие;
- narrow viewport и keyboard smoke проходят;
- реальные результаты записаны в state-документы до merge.

## Exit criteria

- Hermes: `PASS` или только неблокирующие замечания;
- smoke зафиксирован в `state/progress.md` и `state/handoff.md`;
- опубликованный SHA проверен;
- PR mergeable и merged;
- только после этого начинается следующий runtime slice.
...
1. Завершить и смержить PR106 после Hermes smoke.
2. Обновить `state/current-focus.md` на Slice A1.
3. Подготовить английский Codex prompt только для A1.
4. Не смешивать в A1 validation, responsive tables, dashboard, tax/margin, restore или packaging.
