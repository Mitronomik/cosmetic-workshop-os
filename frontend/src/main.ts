...
function orderLifecycleButtons(o: Order) { if (!o.is_active || o.status==='archived') return ''; const busy=orderAnyOperationBusy(o.id); if (o.status==='cancelled') return `<button class="secondary-action compact danger-action" type="button" data-action="archive-order" data-id="${o.id}" ${busy?'disabled':''}>Архивировать</button>`; return `<button class="secondary-action compact danger-action" type="button" data-action="cancel-order" data-id="${o.id}" ${busy?'disabled':''}>Отменить заказ</button><button class="secondary-action compact danger-action" type="button" data-action="archive-order" data-id="${o.id}" ${busy?'disabled':''}>Архивировать</button>`; }
...
function orderDetailPanel(o: Order) {
  ...
  const operationBusy = readinessBusy || writeBusy;
...
function orderProductionPanel(o: Order) {
  ...
  return renderOrderProductionGate({ orderId:o.id, readiness, hasCachedReadiness:Boolean(ordersState.readinessByOrderId[o.id]), confirming, loading, blockedByOperation:orderReadinessBusy(o.id)||orderBoundOperationActive(orderOperationStates(),o.id,['cancel','archive']), notes, error:orderOperationErrorFor(ordersState.productionErrorInfo,o.id) }, escapeHtml);
}
...
function finishProductionOwner(snapshot: OrderRequestSnapshot, id: number){ if(!productionOwnerMatches(snapshot,id)) return false; ordersState.productionRequestOwner=null; ordersState.productionLoadingOrderId=null; ordersState.productionConfirmingOrderId=null; return true; }
