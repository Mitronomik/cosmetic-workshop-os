...
type SettingsDefinition = { id: string; title: string; status: SettingStatus; editable_now: boolean; affects_calculations: boolean; affects_historical_data: boolean; requires_backend_service: boolean; description: string; safety_note: string };
...
function settingsDecisionMatrix(data: SettingsStatusResponse) { return `<section class="card data-card"><p class="card-kicker">Решение на будущее</p><h2>Матрица будущих настроек</h2><p class="next-step">В PR96 редактируется только безопасный профиль мастерской. Расчетные настройки остаются закрыты.</p>${data.setting_groups.map(settingsGroupMarkup).join('')}</section>`; }
