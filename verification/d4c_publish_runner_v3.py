...
GENERATED_BUILD_DIR = Path("backend/build")
...
if GENERATED_BUILD_DIR.exists():
    raise SystemExit("backend/build unexpectedly exists on the exact D4-C base")
...
# `pip install ./backend[test]` invokes setuptools and leaves backend/build as
# untracked wheel-build output. It was proven absent on the exact base above,
# so remove only that generated directory before the repository scope gate.
if GENERATED_BUILD_DIR.exists():
    shutil.rmtree(GENERATED_BUILD_DIR)
if GENERATED_BUILD_DIR.exists():
    raise SystemExit("failed to remove generated backend/build output")
...
actual = sorted(set(filter(None, [*tracked, *untracked])))
...
if output("git", "hash-object", "launcher/runtime.py") != "7cca822944a335e03e196be6d9def8817267205e":
    raise SystemExit("protected launcher/runtime.py changed before commit")
if output("git", "hash-object", "frontend/src/main.ts") != "ea98a76638bddcb5a92b9ba31941508f8a816d42":
    raise SystemExit("protected frontend/src/main.ts changed before commit")
