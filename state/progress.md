...
`C1 — COMPLETED`. `C2 — COMPLETED`. `C3-I`, `C3-II-A`, `C3-II-B1`, `C3-II-B2` and `C3-II-B3` are all `DONE — MERGED AND EXACT-HEAD VERIFIED`. `CR-004`, `CR-006` and `CR-009 — ACCEPTED AND IMPLEMENTED`. `C3 artifact-finalization hardening — DONE — MERGED AND EXACT-HEAD VERIFIED` (PR #168). `C3 — COMPLETED — MERGED, EXACT-HEAD VERIFIED AND HARDENED`. `CR-010 — ACCEPTED — NOT IMPLEMENTED`. `C4 — ACTIVE`. `C4 product decision — COMPLETE`. `C4 implementation — NOT STARTED`. `Restore — NOT IMPLEMENTED`. Product release readiness — **NOT CLAIMED**.

## Current next step

- After this documentation pull request merges, implement only `C4-I — Launcher-owned restore safety engine`. It is `AUTHORIZED AFTER THIS DOCUMENTATION PR MERGES — NOT IMPLEMENTED` and must not be started from this unmerged documentation branch.
- Do not implement `C4-II`, `C4-III`, packaging, the updater or release smoke through `C4-I`.
- `CR-004`, `CR-006` and `CR-009` are accepted and implemented; no change request remains in `needs evidence`.
- Restore remains **not implemented**; macOS packaging, the safe packaged update flow and the full release-candidate smoke remain **not completed**.
