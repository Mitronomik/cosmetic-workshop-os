...
function confirmProduction(id:number){
  ...
  ordersState.productionError=''; ordersState.productionErrorInfo=null; delete ordersState.productionRefreshWarningByOrderId[id];
  ...
  produceOrder(id, ordersState.productionNotesByOrderId[id]).then((batch)=>{
    ...
    if (!orderRequestCanApply(snapshot) || ordersState.selectedOrder?.id !== id) { finishProductionOwner(snapshot,id); render(); return undefined; }
    ...
    ordersState.productionByOrderId[id]=batch; delete ordersState.readinessByOrderId[id]; ordersState.productionConfirmingOrderId=null; ordersState.productionError=''; ordersState.productionErrorInfo=null; ordersState.productionNotesByOrderId[id]='';
    ...
    const [orders, updated]=payload.result; ordersState.items=orders.orders; ordersState.selectedOrder=updated; finishProductionOwner(snapshot,id); render();
  }).catch((e)=>{
    ...
    if(showError){
      if(alreadySucceeded){ ordersState.productionRefreshWarningByOrderId[id]='Изготовление выполнено, но текущие списки не удалось обновить. Обновите заказ безопасно; повторять изготовление не нужно.'; ordersState.productionConfirmingOrderId=null; announceAssertive(ordersState.productionRefreshWarningByOrderId[id]); }
      else { const status=typeof e==='object'&&e&&'status' in e?Number((e as {status?:number}).status):undefined; const presentation=productionConfirmationFailurePresentation({status,message:e instanceof Error?e.message:'',networkFailure:e instanceof TypeError,code:apiErrorCode(e)}); if(presentation.invalidateReadiness) delete ordersState.readinessByOrderId[id]; if(presentation.closeConfirmation) ordersState.productionConfirmingOrderId=null; if(presentation.requireRefreshBeforeRetry) ordersState.productionUncertainByOrderId[id]=true; ordersState.productionError=presentation.message; ordersState.productionErrorInfo=orderOperationError(id, ordersState.productionError); announceAssertive(ordersState.productionError); }
    }
    render();
  });
}
...
function humanProductionError(e: unknown) { const status = typeof e === 'object' && e && 'status' in e ? Number((e as { status?: number }).status) : undefined; return productionConfirmationFailurePresentation({ status, message: e instanceof Error ? e.message : '', networkFailure: e instanceof TypeError, code: apiErrorCode(e) }).message; }
