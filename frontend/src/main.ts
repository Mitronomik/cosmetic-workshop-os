...
import { transitionPurchaseSuggestionsRouteOwnership } from './purchase-suggestions-route.js';
...
function updateRouteOwnership(previousSection: NavigationSection | null, nextSection: NavigationSection) {
  ...
  transitionPurchaseSuggestionsRouteOwnership({ previousSection, nextSection, runtime: { enter: () => purchaseSuggestionsRuntime.enter(), leave: () => purchaseSuggestionsRuntime.leave() } });
  if (previousSection === 'Закупки' && nextSection !== 'Закупки') purchaseReferenceController.leaveRoute();
}
