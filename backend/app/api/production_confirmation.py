...
from app.domain.production_tax_context import parse_expected_tax_rate_context
...
from app.services.production_confirmation import ProductionConfirmationLifecycleError, ProductionConfirmationReadinessError, ProductionConfirmationRequiredError, ProductionConfirmationService, ProductionConfirmationTaxRateContextStaleError
...
def produce_order(order_id: int, payload: ProductionConfirmRequest):
    try:
        expected = parse_expected_tax_rate_context(payload.expected_tax_rate_percent, payload.expected_tax_rate_effective_at)
        return _response(ProductionConfirmationService().produce_order(order_id, payload.confirm, payload.notes, expected_tax_rate=expected))
    except ProductionConfirmationTaxRateContextStaleError as exc:
        raise HTTPException(409, detail=_error(exc.code, str(exc), next_action="Запустите проверку готовности заново и подтвердите изготовление ещё раз.")) from exc
...
def _response(detail) -> ProductionBatchDetailResponse:
    ...
    return ProductionBatchDetailResponse(
        ...
        component_cost=_s(b.component_cost), packaging_cost=_s(b.packaging_cost), other_cost=str(b.other_cost), total_cost=_s(b.total_cost), sale_price=_s(b.sale_price), tax=_s(b.tax), margin=_s(b.margin), margin_percent=_s(b.margin_percent), tax_rate_percent_snapshot=b.tax_rate_percent_snapshot, tax_rate_effective_at_snapshot=b.tax_rate_effective_at_snapshot, produced_at=b.produced_at, notes=b.notes, created_at=b.created_at,
