...
from app.schemas.orders import OrderRequest, OrderResponse, OrdersResponse
...
def create_order(payload: OrderRequest):
    return _handle_write(lambda: _response(OrderService().create(OrderDraft.create(**payload.model_dump()))))

@router.get("/orders", response_model=OrdersResponse)
def list_orders(include_inactive: bool=True, status: str | None=None, client_id: int | None=None):
    try: return OrdersResponse(orders=[_response(o) for o in OrderService().list_orders(include_inactive=include_inactive, status=status, client_id=client_id)])
...
def get_order(order_id:int):
    try: return _response(OrderService().get_by_id(order_id))
    except DomainValidationError as exc: raise HTTPException(422, detail=exc.issue.__dict__) from exc
    except OrderNotFoundError as exc: raise HTTPException(404, detail="Order was not found.") from exc

@router.put("/orders/{order_id}", response_model=OrderResponse)
def update_order(order_id:int, payload: OrderRequest):
...
def _handle_write(fn):
    ...
    except (ClientNotFoundError, RecipeVersionNotFoundError, ClientRecipeNotFoundError, PackagingItemNotFoundError, OrderNotFoundError) as exc: raise HTTPException(404, detail="Linked record was not found.") from exc
    except (OrderClientInactiveError, OrderClientRecipeInactiveError, OrderClientRecipeMismatchError, OrderPackagingInactiveError, OrderLifecycleError) as exc: raise HTTPException(409, detail=str(exc)) from exc
