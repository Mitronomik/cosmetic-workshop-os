...
**Current merged baseline.** `origin/main` is `7369e7f133f0ce02aea5f2021cbb0e14104b7b34` (the PR #156 merge commit), and the current accepted merged backend result is `883 passed / 0 failed / 0 skipped`. The backend count is unchanged from PR #154 because PR #155 and PR #156 were **documentation-only**. The earlier values `d432fcaee52a16a4f8b609ec160cf3fa2b33d013` (PR #154) and `c3a3a7b8db06fe85290216113b784123ed9b6b30` (PR #152) are **historical**.
...
**The C2 financial contract is accepted and partly implemented; C2 is not complete.** `CR-008 — Decide C2 financial estimates and immutable production snapshots` records the accepted calculation and immutable-snapshot contract. `C2-I` — the backend financial readiness estimate — is **merged and `DONE`** (PR #151), and `C2-II` — the transactional production financial snapshots, including migration `0019` — is **merged and `DONE`** (PR #152, merge commit `c3a3a7b8db06fe85290216113b784123ed9b6b30`, merged `2026-07-28T09:00:50Z`). The former `C2-III` planning umbrella is subdivided into exactly two runtime slices: `C2-III-A` — Order and `ProductionBatch` financial presentation — is **merged and `DONE`** (PR #154, final reviewed head `ef1103811a8f062f9129bfb465a98e0cfa388935`, merge commit `d432fcaee52a16a4f8b609ec160cf3fa2b33d013`, merged `2026-07-28T13:05:34Z`), and `C2-III-B` — snapshot-backed reports and report documents — is **`IMPLEMENTED ON PR #157 BRANCH — NOT MERGED`** (branch `codex/c2-iii-b-snapshot-backed-reports`). The financial presentation exists in the merged Orders and `ProductionBatch` UI; snapshot-backed Reports exist **on the PR #157 branch only**, so on merged `main` reports still derive margin from paired sale price and cost and will do so until PR #157 merges. `C2-III-B` is not `DONE` and C2 is **not complete**. Contract: `docs/decisions/0012-c2-financial-calculation-snapshots.md`. **The product is not release-ready.**
...
The product is **not** release-ready. The remaining release obligations are: the `CR-004` SQLite backup transaction-consistency investigation; the Restore product decision and implementation; final macOS packaging and user-ready launch; installation verification; the packaged update flow and update smoke; the full release-candidate smoke; the C2 financial implementation, whose contract is decided as `CR-008` and whose `C2-I`, `C2-II` and `C2-III-A` slices are merged, with `C2-III-B` implemented on the PR #157 branch and not merged, so C2 is **not complete**; C3 and C4, which remain **inactive** unless separately authorized; and continuing documentation accuracy. The C1 tax-setting implementation is no longer an outstanding obligation: it is merged and `DONE`.
...
The current focus is the **C2 financial implementation sequence**. `R3`, `R2`, and `R4` are all merged and DONE, the backend baseline correction gate is **DONE**, `CR-007` is accepted, and `C1-I` is merged and **DONE**. `CR-008` records the accepted C2 calculation and immutable-snapshot contract; `C2-I` (PR #151), `C2-II` (PR #152) and `C2-III-A` (PR #154) are merged and `DONE`, and `main` is now `7369e7f133f0ce02aea5f2021cbb0e14104b7b34`. **One runtime implementation slice is open and unmerged: `C2-III-B`, PR #157.** The former `C2-III` umbrella is subdivided into `C2-III-A` (Order and `ProductionBatch` financial presentation — merged and `DONE`) and `C2-III-B` (snapshot-backed reports and report documents — `IMPLEMENTED ON PR #157 BRANCH — NOT MERGED`). Reports on merged `main` are not snapshot-backed until PR #157 merges. C2 is **not complete**.
...
`R4 — Canonical backup/export filename reason normalization` delivered that contract as one bounded slice covering both nodes through a single shared backend helper, and it is **merged and DONE** as PR #146. `CR-006` is a `needs evidence` row and is **not active**. `CR-004` remains separate and inactive. C1 has an accepted product contract (`CR-007`) and a merged implementation (`C1-I`, PR #149), so C1 is **DONE**. C2 has an accepted product contract (`CR-008`) and a **partial implementation**: `C2-I`, `C2-II` and `C2-III-A` are merged, and `C2-III-B` is implemented on the PR #157 branch but not merged, so C2 is **not complete**. C3 and C4 remain inactive. Restore remains open, packaging/install/update work and the full release-candidate smoke remain open, and **product release readiness is not claimed**. Contract: `state/current-focus.md` and `docs/implementation-plan.md`. Evidence: `docs/backend-baseline-failure-triage.md`.
