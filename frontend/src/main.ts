...
function orderProductionPanel(o: Order) {
  ...
  return renderOrderProductionGate({ orderId:o.id, readiness, hasCachedReadiness:Boolean(ordersState.readinessByOrderId[o.id]), confirming, loading, blockedByOperation:orderReadinessBusy(o.id), persistentWriteActive, notes, error:orderOperationErrorFor(ordersState.productionErrorInfo,o.id), recoveryAction: ordersState.productionRecoveryByOrderId[o.id] || '', uncertain: Boolean(ordersState.productionUncertainByOrderId[o.id]) }, escapeHtml);
}
...
function openProductionConfirmation(id:number){ const order=ordersState.selectedOrder?.id===id ? ordersState.selectedOrder : ordersState.items.find(i=>i.id===id) ?? null; const readiness=order ? currentOrderReadiness(order) : null; if(!canOpenOrderProductionConfirmation(orderFormMutationActive()||orderAnyOperationBusy(id)||orderPersistentWriteBusy(), order, readiness)) return; ordersState.productionConfirmingOrderId=id; ordersState.productionError=''; ordersState.productionErrorInfo=null; if(!(id in ordersState.productionNotesByOrderId)) ordersState.productionNotesByOrderId[id]=''; render(); focusOrderProduction('confirmation'); }
...
function confirmProduction(id:number){
  ...
  if(ordersState.productionUncertainByOrderId[id]) { ordersState.productionError='Перед повторной попыткой обновите заказ и историю производства: исход прошлого запроса неизвестен.'; ordersState.productionErrorInfo=orderOperationError(id, ordersState.productionError); render(); return; }
  ...
  ordersState.productionError=''; ordersState.productionErrorInfo=null; delete ordersState.productionRefreshWarningByOrderId[id]; delete ordersState.productionRecoveryByOrderId[id];
  ...
  produceOrder(id, ordersState.productionNotesByOrderId[id]).then((batch)=>{
    ...
    ordersState.productionByOrderId[id]=batch; delete ordersState.readinessByOrderId[id]; delete ordersState.productionUncertainByOrderId[id]; delete ordersState.productionRecoveryByOrderId[id]; ordersState.productionError=''; ordersState.productionErrorInfo=null; ordersState.productionNotesByOrderId[id]=''; if (ordersState.selectedOrder?.id===id) ordersState.productionConfirmingOrderId=null;
    announcePolite('Изготовление подтверждено. Производственная партия создана.');
    if (ordersState.selectedOrder?.id!==id || !orderRequestCanApply(snapshot)) { finishProductionOwner(snapshot,id); render(); return undefined; }
    ...
    if(alreadySucceeded){ if(showError){ ordersState.productionRefreshWarningByOrderId[id]='Изготовление выполнено, но текущие списки не удалось обновить. Обновите заказ безопасно; повторять изготовление не нужно.'; ordersState.productionConfirmingOrderId=null; announceAssertive(ordersState.productionRefreshWarningByOrderId[id]); focusOrderProduction('success'); } }
    else { const presentation=productionConfirmationFailurePresentation(extractProductionApiFailure(e)); if(presentation.invalidateReadiness) delete ordersState.readinessByOrderId[id]; if(presentation.closeConfirmation && ordersState.selectedOrder?.id===id) ordersState.productionConfirmingOrderId=null; if(presentation.requireRefreshBeforeRetry) ordersState.productionUncertainByOrderId[id]=true; ordersState.productionError=presentation.message; ordersState.productionErrorInfo=orderOperationError(id, ordersState.productionError); ordersState.productionRecoveryByOrderId[id]=presentation.nextAction; if(showError){ announceAssertive(ordersState.productionError); setTimeout(()=>focusOrderProduction('failure'),0); } }
    render();
  });
}
...
function reconcileProductionOutcome(id:number){ if(orderFormMutationActive() || orderPersistentWriteBusy()) return; ordersState.productionRecoveryByOrderId[id]='Проверяем заказ и производственную партию по локальным данным…'; render(); Promise.allSettled([getOrder(id), getProductionBatchByOrder(id)]).then(([orderResult,batchResult])=>{ const order=orderResult.status==='fulfilled'?orderResult.value:null; const batch=batchResult.status==='fulfilled'?batchResult.value:null; if(batch && productionResponseBelongsToOrder(batch,id)){ ordersState.productionByOrderId[id]=batch; delete ordersState.readinessByOrderId[id]; delete ordersState.productionUncertainByOrderId[id]; delete ordersState.productionRecoveryByOrderId[id]; ordersState.productionConfirmingOrderId=null; ordersState.productionError=''; ordersState.productionErrorInfo=null; if(order && ordersState.selectedOrder?.id===id) ordersState.selectedOrder=order; render(); focusOrderProduction('success'); return; } const batchMissing=batchResult.status==='rejected' && typeof batchResult.reason==='object' && batchResult.reason && 'status' in batchResult.reason && Number((batchResult.reason as {status?:number}).status)===404; if(order && batchMissing && order.status!=='produced' && order.is_active && !orderProductionClosed(order)){ delete ordersState.productionUncertainByOrderId[id]; delete ordersState.readinessByOrderId[id]; ordersState.productionConfirmingOrderId=null; ordersState.productionError='Изготовление не найдено по актуальным данным. Запустите проверку готовности заново перед новой попыткой.'; ordersState.productionErrorInfo=orderOperationError(id,ordersState.productionError); ordersState.productionRecoveryByOrderId[id]='Нажмите «Проверить изготовление», чтобы получить новый текущий результат готовности.'; if(ordersState.selectedOrder?.id===id) ordersState.selectedOrder=order; render(); focusOrderProduction('failure'); return; } ordersState.productionUncertainByOrderId[id]=true; ordersState.productionError='Исход изготовления всё ещё не подтверждён. Повторите безопасную проверку результата.'; ordersState.productionErrorInfo=orderOperationError(id,ordersState.productionError); ordersState.productionRecoveryByOrderId[id]='Проверка должна получить и заказ, и производственную партию. До этого повторное изготовление заблокировано.'; render(); focusOrderProduction('failure'); }); }
