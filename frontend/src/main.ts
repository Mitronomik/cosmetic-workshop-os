...
let purchaseReferenceState: { requestId: number; routeGeneration: number; status: 'idle'|'loading'|'ready'|'error'; error: string; ingredientsLoaded: boolean; packagingLoaded: boolean } = { requestId: 0, routeGeneration: 0, status: 'idle', error: '', ingredientsLoaded: false, packagingLoaded: false };
const purchaseSuggestionsRuntime = createPurchaseSuggestionsRuntime<PurchaseSuggestionResponse>(purchaseSuggestionsLifecycle, { read: (filters) => getPurchaseSuggestions(filters).then((response) => response.purchase_suggestions), regenerate: regeneratePurchaseSuggestions, create: createManualPurchaseSuggestion, update: updatePurchaseSuggestion, markPurchased: markPurchaseSuggestionPurchased, dismiss: dismissPurchaseSuggestion, ownsRoute: () => activeSection === 'Закупки', render: () => { if (activeSection === 'Закупки') render(); }, announce: (message, kind) => kind === 'assertive' ? announceAssertive(message) : announcePolite(message), focus: (key) => focusPurchaseTarget(key), onValidCreate: () => { purchaseSuggestionsState.showManualForm = false; purchaseSuggestionsState.manualForm = emptyManualPurchaseSuggestionForm(); }, onValidUpdate: () => { purchaseSuggestionsState.editingSuggestionId = null; purchaseSuggestionsState.editForm = { recommended_quantity: '', unit: '', notes: '' }; } });
...
function loadSectionData(section: NavigationSection) {
  ...
  if (section === 'Закупки') purchaseSuggestionsRuntime.enter();
...
function bindEvents(root: HTMLElement) {
  ...
  root.querySelectorAll<HTMLButtonElement>('[data-action="reload-purchase-suggestions"]').forEach((button) => button.addEventListener('click', () => loadPurchaseSuggestions(true, purchaseSuggestionsLifecycle.displayedFilters(), 'refresh')));
  root.querySelectorAll<HTMLButtonElement>('[data-action="regenerate-purchase-suggestions"]').forEach((button) => button.addEventListener('click', runPurchaseSuggestionRegeneration));
  ...
  root.querySelector<HTMLSelectElement>('[data-action="filter-purchase-status"]')?.addEventListener('change', (event) => { loadPurchaseSuggestions(true, { ...purchaseSuggestionsLifecycle.displayedFilters(), status: (event.currentTarget as HTMLSelectElement).value as PurchaseSuggestionStatusFilter }, 'filter'); });
  root.querySelector<HTMLSelectElement>('[data-action="filter-purchase-reason"]')?.addEventListener('change', (event) => { loadPurchaseSuggestions(true, { ...purchaseSuggestionsLifecycle.displayedFilters(), reason: (event.currentTarget as HTMLSelectElement).value as PurchaseSuggestionReasonFilter }, 'filter'); });
  root.querySelector<HTMLSelectElement>('[data-action="filter-purchase-item-type"]')?.addEventListener('change', (event) => { loadPurchaseSuggestions(true, { ...purchaseSuggestionsLifecycle.displayedFilters(), itemType: (event.currentTarget as HTMLSelectElement).value as PurchaseSuggestionItemTypeFilter }, 'filter'); });
  root.querySelectorAll<HTMLButtonElement>('[data-action="reset-purchase-filters"]').forEach((button) => button.addEventListener('click', () => { purchaseSuggestionsLifecycle.setSearch(''); loadPurchaseSuggestions(true, DEFAULT_PURCHASE_FILTERS, 'filter'); }));
  root.querySelectorAll<HTMLButtonElement>('[data-action="open-manual-purchase-form"]').forEach((button) => button.addEventListener('click', openManualPurchaseSuggestionForm));
  root.querySelector<HTMLButtonElement>('[data-action="cancel-manual-purchase-form"]')?.addEventListener('click', () => { if (purchaseView().controlsDisabled) return; purchaseSuggestionsState.showManualForm = false; purchaseSuggestionsState.manualForm = emptyManualPurchaseSuggestionForm(); render(); });
  root.querySelector<HTMLButtonElement>('[data-action="reload-purchase-references"]')?.addEventListener('click', loadPurchaseReferenceData);
...
function purchaseFeedbackMarkup() { const f = purchaseSuggestionsLifecycle.state.feedback; return `${f.neutral ? feedbackMessage('neutral', f.neutral) : ''}${f.success ? feedbackMessage('success', f.success) : ''}${f.refreshWarning ? feedbackMessage('warning', f.refreshWarning) : ''}${f.mutationError ? feedbackMessage('error', f.mutationError) : ''}`; }
...
function loadPurchaseReferenceData() { const requestId = purchaseReferenceState.requestId + 1; const routeGeneration = purchaseSuggestionsLifecycle.state.routeGeneration; purchaseReferenceState = { ...purchaseReferenceState, requestId, routeGeneration, status: 'loading', error: '' }; render(); Promise.allSettled([getIngredients(), getPackagingItems()]).then((results) => { if (purchaseReferenceState.requestId !== requestId || purchaseReferenceState.routeGeneration !== routeGeneration || activeSection !== 'Закупки') return; const [ingredients, packaging] = results; if (ingredients.status === 'fulfilled') { ingredientsState.items = ingredients.value.ingredients; purchaseReferenceState.ingredientsLoaded = true; } if (packaging.status === 'fulfilled') { packagingItemsState.items = packaging.value.packaging_items; purchaseReferenceState.packagingLoaded = true; } const failed = results.some((r) => r.status === 'rejected'); purchaseReferenceState.status = failed ? 'error' : 'ready'; purchaseReferenceState.error = failed ? 'Не удалось загрузить один из справочников для ручного предложения. Список закупок сохранён, черновик не потерян.' : ''; render(); }); }
...
function updateAlertsRouteOwnership(previousSection: NavigationSection, nextSection: NavigationSection) {
  transitionAlertsRouteOwnership(alertsLifecycle, previousSection, nextSection);
  if (previousSection === 'Закупки' && nextSection !== 'Закупки') purchaseSuggestionsRuntime.leave();
}
...
function navigateToSection(section: NavigationSection | undefined) { if (!section) return; const previousSection = activeSection; activeSection = section; updateAlertsRouteOwnership(previousSection, activeSection); clearRouteOwnedFeedbackOnNavigation(activeSection); window.history.pushState({}, '', pathForSection(activeSection)); loadSectionData(activeSection); render(); }
...
window.addEventListener('popstate', () => { const previousSection = activeSection; activeSection = sectionFromLocation(); updateAlertsRouteOwnership(previousSection, activeSection); clearRouteOwnedFeedbackOnNavigation(activeSection); loadSectionData(activeSection); render(); });
ensureAnnouncementRegions();
updateAlertsRouteOwnership('Главная', activeSection);
render();
