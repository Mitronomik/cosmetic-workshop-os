...
C2 was not implemented inside the C1 implementation PR. Its product contract is `CR-008` / `docs/decisions/0012-c2-financial-calculation-snapshots.md`, which refines the cost and margin formulas without contradicting this accepted C1 tax decision. C2 is gated on its own slice sequence instead: `C2-I` (PR #151) and `C2-II` (PR #152) are merged, `C2-III-A` is authorized and not implemented, and `C2-III-B` stays `PLANNED — BLOCKED` behind it. C2 is **not complete**.
...
>
> For the current status, see § 13 *Boundary between C1 and C2* and the active C1/C2 lifecycle list near the top of this document: `C1-I` merged as PR #149, `C2-I` as PR #151, and `C2-II` as PR #152; `C2-III-A` is authorized and not implemented, `C2-III-B` is `PLANNED — BLOCKED`, and C2 is not complete. The durable C2 contract is `docs/decisions/0012-c2-financial-calculation-snapshots.md`, and the current lifecycle state of record is `state/current-focus.md`.
