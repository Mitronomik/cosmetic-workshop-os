...
Статус: `IN PROGRESS — A3.4 client recipe structured validation`
...
A3.4 is the current Client Recipe structured-validation slice on `/client-recipes`: Client Recipe create and composition update use shared structured backend validation, indexed composition paths, structural row-error invalidation, duplicate-submit protection, create refresh-failure separation, and authoritative composition `PUT` responses. Slice A3 remains in progress because Client Wishes, Client Feedback, Orders, and Production remain separate future work.
