...
function loadClientCardData(clientId: number) { clientCardState = { ...emptyClientCardState(), clientId }; render(); refreshClientWishes(); refreshClientFeedback(); clientCardState.recipesStatus = 'loading'; getClientRecipes(true).then((response) => { if (clientCardState.clientId !== clientId) return; clientCardState.recipes = response.client_recipes.filter((recipe) => recipe.client_id === clientId); clientCardState.recipesStatus = 'ready'; render(); }).catch(() => { clientCardState.recipesStatus = 'error'; render(); }); }
function refreshClientWishes() { const clientId = clientCardState.clientId; if (!clientId) return Promise.resolve(); clientCardState.wishesStatus = 'loading'; render(); return fetchClientWishes(clientId, clientCardState.includeArchivedWishes).then((wishes) => { if (clientCardState.clientId !== clientId) return; clientCardState.wishes = wishes; clientCardState.wishesStatus = 'ready'; render(); }).catch(() => { clientCardState.wishesStatus = 'error'; clientCardState.wishError = 'Не удалось загрузить пожелания клиента. Обновите карточку и попробуйте ещё раз.'; render(); }); }
function refreshClientFeedback() { const clientId = clientCardState.clientId; if (!clientId) return Promise.resolve(); clientCardState.feedbackStatus = 'loading'; render(); return fetchClientFeedback(clientId).then((feedback) => { if (clientCardState.clientId !== clientId) return; clientCardState.feedback = feedback; clientCardState.feedbackStatus = 'ready'; render(); }).catch(() => { clientCardState.feedbackStatus = 'error'; clientCardState.feedbackError = 'Не удалось загрузить обратную связь клиента. Обновите карточку и попробуйте ещё раз.'; render(); }); }
function toggleClientWishForm() { clientCardState.showWishForm = !clientCardState.showWishForm; clientCardState.wishError = ''; render(); }
...
function submitClientWishForm(event: SubmitEvent) { event.preventDefault(); const clientId = clientCardState.clientId; if (!clientId) return; const data = new FormData(event.currentTarget as HTMLFormElement); const payload: ClientWishCreatePayload = { title: String(data.get('title') ?? '').trim(), description: String(data.get('description') ?? '').trim(), category: String(data.get('category') ?? 'other'), priority: String(data.get('priority') ?? 'normal') as ClientWishPriority, client_recipe_id: nullableNumber(String(data.get('client_recipe_id') ?? '')) }; if (!payload.title) { clientCardState.wishError = 'Укажите краткое пожелание клиента.'; render(); return; } clientCardState.savingWish = true; clientCardState.wishError = ''; render(); createClientWish(clientId, payload).then(() => { clientCardState.showWishForm = false; clientCardState.wishForm = emptyClientWishForm(); clientCardState.savingWish = false; return refreshClientWishes(); }).catch((error) => { clientCardState.savingWish = false; clientCardState.wishError = humanWishFeedbackError(error, 'Не удалось сохранить пожелание. Проверьте поля и попробуйте ещё раз.'); render(); }); }
function changeClientWishStatus(wishId: number, status: ClientWishStatus) { if (!['open','planned','resolved'].includes(status)) return; clientCardState.changingWishId = wishId; clientCardState.wishError = ''; render(); updateClientWishStatus(wishId, status as 'open' | 'planned' | 'resolved').then(() => { clientCardState.changingWishId = null; return refreshClientWishes(); }).catch(() => { clientCardState.changingWishId = null; clientCardState.wishError = 'Не удалось изменить статус пожелания. Обновите карточку клиента и попробуйте ещё раз.'; render(); }); }
function archiveClientWishFromCard(wishId: number) { if (!window.confirm('Архивировать пожелание клиента? Оно останется в истории.')) return; clientCardState.archivingWishId = wishId; clientCardState.wishError = ''; render(); archiveClientWish(wishId).then(() => { clientCardState.archivingWishId = null; return refreshClientWishes(); }).catch(() => { clientCardState.archivingWishId = null; clientCardState.wishError = 'Не удалось архивировать пожелание. Попробуйте ещё раз.'; render(); }); }
function toggleClientFeedbackForm() { clientCardState.showFeedbackForm = !clientCardState.showFeedbackForm; clientCardState.feedbackError = ''; render(); }
...
function submitClientFeedbackForm(event: SubmitEvent) { event.preventDefault(); const clientId = clientCardState.clientId; if (!clientId) return; const data = new FormData(event.currentTarget as HTMLFormElement); const ratingRaw = String(data.get('rating') ?? '').trim(); const payload: ClientFeedbackCreatePayload = { feedback_type: String(data.get('feedback_type') ?? 'note'), sentiment: String(data.get('sentiment') ?? 'neutral'), rating: ratingRaw ? Number(ratingRaw) : null, text: String(data.get('text') ?? '').trim(), follow_up_needed: data.get('follow_up_needed') === 'on', follow_up_note: String(data.get('follow_up_note') ?? '').trim(), occurred_at: String(data.get('occurred_at') ?? '').trim() || null, client_recipe_id: nullableNumber(String(data.get('client_recipe_id') ?? '')) }; if (!payload.text) { clientCardState.feedbackError = 'Запишите текст отзыва клиента.'; render(); return; } clientCardState.savingFeedback = true; clientCardState.feedbackError = ''; render(); createClientFeedback(clientId, payload).then(() => { clientCardState.showFeedbackForm = false; clientCardState.feedbackForm = emptyClientFeedbackForm(); clientCardState.savingFeedback = false; return refreshClientFeedback(); }).catch((error) => { clientCardState.savingFeedback = false; clientCardState.feedbackError = humanWishFeedbackError(error, 'Не удалось сохранить отзыв. Проверьте поля и попробуйте ещё раз.'); render(); }); }
