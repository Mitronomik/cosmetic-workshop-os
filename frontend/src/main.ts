...
let stockMovementDetailToken = 0;
...
function bindEvents(root: HTMLElement) {
  ...
  root.querySelector<HTMLButtonElement>('[data-action="reload-packaging-items"]')?.addEventListener('click', () => { if (packagingItemSubmitting) return; loadPackagingItems(true); });
  ...
  root.querySelector<HTMLInputElement>('[data-action="filter-packaging-search"]')?.addEventListener('input', (event) => { if (packagingItemSubmitting) return; updatePackagingFilterSearch(event.currentTarget as HTMLInputElement); });
  root.querySelector<HTMLSelectElement>('[data-action="filter-packaging-category"]')?.addEventListener('change', (event) => { if (packagingItemSubmitting) return; packagingItemsState.filters.categoryId = catalogCategoryFilterValue((event.currentTarget as HTMLSelectElement).value); render(); });
  root.querySelector<HTMLSelectElement>('[data-action="filter-packaging-kind"]')?.addEventListener('change', (event) => { if (packagingItemSubmitting) return; packagingItemsState.filters.systemType = (event.currentTarget as HTMLSelectElement).value; render(); });
  root.querySelector<HTMLSelectElement>('[data-action="filter-packaging-status"]')?.addEventListener('change', (event) => { if (packagingItemSubmitting) return; packagingItemsState.filters.status = (event.currentTarget as HTMLSelectElement).value as CatalogStatusFilter; render(); });
  root.querySelector<HTMLSelectElement>('[data-action="add-packaging-tag-filter"]')?.addEventListener('change', (event) => { if (packagingItemSubmitting) return; addPackagingTagFilter((event.currentTarget as HTMLSelectElement).value); });
  root.querySelectorAll<HTMLButtonElement>('[data-action="remove-packaging-tag-filter"]').forEach((button) => button.addEventListener('click', () => { if (packagingItemSubmitting) return; removePackagingTagFilter(Number(button.dataset.id)); }));
  root.querySelectorAll<HTMLButtonElement>('[data-action="clear-packaging-filter"]').forEach((button) => button.addEventListener('click', () => { if (packagingItemSubmitting) return; clearPackagingFilter(button.dataset.filter ?? ''); }));
  root.querySelectorAll<HTMLButtonElement>('[data-action="reset-packaging-filters"]').forEach((button) => button.addEventListener('click', () => { if (packagingItemSubmitting) return; packagingItemsState.filters = emptyCatalogBrowserFilters(); render(); }));
  root.querySelector<HTMLFormElement>('[data-form="packaging-catalog-category"]')?.addEventListener('submit', (event) => { if (packagingItemSubmitting) { event.preventDefault(); return; } submitPackagingCatalogCategoryForm(event); });
  root.querySelector<HTMLFormElement>('[data-form="packaging-catalog-tag"]')?.addEventListener('submit', (event) => { if (packagingItemSubmitting) { event.preventDefault(); return; } submitPackagingCatalogTagForm(event); });
  root.querySelectorAll<HTMLButtonElement>('[data-action="assign-packaging-category"]').forEach((button) => button.addEventListener('click', () => { if (packagingItemSubmitting) return; updatePackagingDraftCategory(Number(button.dataset.id), button.dataset.value ?? ''); }));
  root.querySelectorAll<HTMLInputElement>('[data-action="toggle-packaging-tag"]').forEach((input) => input.addEventListener('change', () => { if (packagingItemSubmitting) return; updatePackagingDraftTag(Number(input.dataset.packagingItemId), Number(input.value), input.checked); }));
  root.querySelector<HTMLButtonElement>('[data-action="apply-packaging-assignment"]')?.addEventListener('click', () => { if (packagingItemSubmitting) return; applyPackagingAssignmentDraft(); });
  root.querySelector<HTMLButtonElement>('[data-action="reset-packaging-assignment"]')?.addEventListener('click', () => { if (packagingItemSubmitting) return; resetPackagingAssignmentDraft(); });
  root.querySelector<HTMLInputElement>('[data-action="search-packaging-category"]')?.addEventListener('input', (event) => { if (packagingItemSubmitting) return; updateCatalogSearch(packagingCatalogControls, 'categorySearch', event.currentTarget as HTMLInputElement); });
  root.querySelector<HTMLInputElement>('[data-action="search-packaging-tags"]')?.addEventListener('input', (event) => { if (packagingItemSubmitting) return; updateCatalogSearch(packagingCatalogControls, 'tagSearch', event.currentTarget as HTMLInputElement); });
  root.querySelector<HTMLButtonElement>('[data-action="toggle-packaging-tags"]')?.addEventListener('click', () => { if (packagingItemSubmitting) return; packagingCatalogControls.showAllTags = !packagingCatalogControls.showAllTags; render(); });
...
function packagingCatalogPanel() {
  ...
  const isDirty = assignmentDraftIsDirty(item, draft);
  const draftNotice = isDirty ? '<p class="page-message">Есть несохранённые изменения</p>' : '';
  return `<section class="card form-card"><p class="card-kicker">Организация каталога тары</p><h2>Группа и метки</h2><p>${helperCopy}</p><div class="catalog-classification">${catalogCategoryPicker({ itemId: item.id, selectedId: draft.catalogCategoryId, categories: packagingItemsState.catalogCategories, state: packagingCatalogControls, disabled: packagingItemSubmitting || packagingItemsState.catalogSaving === 'saving', action: 'assign-packaging-category', searchAction: 'search-packaging-category' })}${catalogTagPicker({ itemId: item.id, selectedIds: draft.catalogTagIds, tags: packagingItemsState.catalogTags, state: packagingCatalogControls, disabled: packagingItemSubmitting || packagingItemsState.catalogSaving === 'saving', toggleAction: 'toggle-packaging-tag', itemDataName: 'packaging-item-id', searchAction: 'search-packaging-tags', showMoreAction: 'toggle-packaging-tags' })}</div>${draftNotice}<div class="actions"><button class="primary-action" type="button" data-action="apply-packaging-assignment" ${packagingItemSubmitting || !isDirty || packagingItemsState.catalogSaving === 'saving' ? 'disabled' : ''}>${packagingItemsState.catalogSaving === 'saving' ? 'Сохраняем…' : 'Применить изменения'}</button><button class="secondary-action" type="button" data-action="reset-packaging-assignment" ${packagingItemSubmitting || !isDirty || packagingItemsState.catalogSaving === 'saving' ? 'disabled' : ''}>Сбросить</button></div>${createControls}<p class="next-step">Группа и метки изменяются как черновик. Нажмите «Применить изменения», чтобы сохранить их.</p></section>`;
}

function packagingCatalogCreateControls() {
  const categoryDisabled = packagingItemSubmitting || packagingItemsState.catalogCreating === 'category' || packagingItemsState.catalogSaving === 'saving';
  const tagDisabled = packagingItemSubmitting || packagingItemsState.catalogCreating === 'tag' || packagingItemsState.catalogSaving === 'saving';
...
function packagingCatalogToolbar(resultCount: number) {
  ...
  const activeTagChips = f.tagIds.map((id) => packagingItemsState.catalogTags.find((tag) => tag.id === id)).filter((tag): tag is CatalogTag => Boolean(tag)).map((tag) => `<span class="tag-chip selected">${escapeHtml(tag.name)} <button type="button" data-action="remove-packaging-tag-filter" ${packagingItemSubmitting ? 'disabled' : ''} data-id="${tag.id}" aria-label="Убрать метку ${escapeHtml(tag.name)}">×</button></span>`).join('');
  const activeFilters = packagingActiveFilterChips();
  return `<section class="card data-card catalog-browser"><p class="card-kicker">Каталог тары</p><h2>Найти тару</h2><div class="catalog-toolbar"><label class="full-span">Поиск<input type="search" data-action="filter-packaging-search" ${packagingItemSubmitting ? 'disabled' : ''} value="${escapeHtml(f.search)}" placeholder="Название, поставщик, заметки, материал или тип тары" /></label><label>Группа<select data-action="filter-packaging-category" ${packagingItemSubmitting ? 'disabled' : ''}>${categoryOptionsHtml}</select></label><label>Метки<select data-action="add-packaging-tag-filter" ${packagingItemSubmitting ? 'disabled' : ''}><option value="">Все метки</option>${availableTags.map((tag) => `<option value="${tag.id}">${escapeHtml(tag.name)}</option>`).join('')}</select></label><label>Тип тары<select data-action="filter-packaging-kind" ${packagingItemSubmitting ? 'disabled' : ''}><option value="" ${f.systemType === '' ? 'selected' : ''}>Все типы</option>${packagingKindValues().map((value) => `<option value="${value}" ${f.systemType === value ? 'selected' : ''}>${packagingKindLabel(value)}</option>`).join('')}</select></label><label>Статус<select data-action="filter-packaging-status" ${packagingItemSubmitting ? 'disabled' : ''}><option value="active" ${f.status === 'active' ? 'selected' : ''}>Активные</option><option value="archived" ${f.status === 'archived' ? 'selected' : ''}>Архив</option><option value="all" ${f.status === 'all' ? 'selected' : ''}>Все</option></select></label></div>${activeTagChips ? `<div class="active-filter-row"><strong>Метки:</strong>${activeTagChips}</div>` : ''}${activeFilters ? `<div class="active-filter-row"><strong>Активные фильтры:</strong>${activeFilters}</div>` : ''}<div class="catalog-summary"><span>Показана тара: ${resultCount} из ${packagingItemsState.items.length}</span><button class="secondary-action compact" type="button" data-action="reset-packaging-filters" ${packagingItemSubmitting ? 'disabled' : ''}>Сбросить фильтры</button></div></section>`;
}
...
function packagingItemsList(items: PackagingItem[]) {
  ...
  if (items.length === 0) return `<section class="card empty-card"><h2>По этим фильтрам тара не найдена.</h2><p>Попробуйте убрать часть условий поиска или вернуться к полному каталогу.</p><button class="secondary-action" type="button" data-action="reset-packaging-filters" ${packagingItemSubmitting ? 'disabled' : ''}>Сбросить фильтры</button></section>`;
...
function mutationDisabled(element: Element | null) {
...
function mutationReadonly(element: Element | null) {
  if (!(element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement)) return;
  if (element.readOnly || element.disabled) return;
  element.readOnly = true;
  element.dataset.mutationReadonly = 'true';
}
function restoreMutationGuards(root: ParentNode = document) {
  root.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>('[data-mutation-readonly="true"]').forEach((el) => { el.readOnly = false; delete el.dataset.mutationReadonly; });
  root.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement | HTMLSelectElement>('[data-mutation-disabled="true"]').forEach((el) => { el.disabled = false; delete el.dataset.mutationDisabled; });
}
...
function addPackagingTagFilter(value: string) { if (packagingItemSubmitting) return; const id = Number(value); if (!id || packagingItemsState.filters.tagIds.includes(id)) return; packagingItemsState.filters.tagIds = [...packagingItemsState.filters.tagIds, id]; render(); }
function removePackagingTagFilter(id: number) { if (packagingItemSubmitting) return; packagingItemsState.filters.tagIds = packagingItemsState.filters.tagIds.filter((tagId) => tagId !== id); render(); }
function clearPackagingFilter(filter: string) { if (packagingItemSubmitting) return; if (filter === 'search') packagingItemsState.filters.search = ''; if (filter === 'category') packagingItemsState.filters.categoryId = ''; if (filter === 'kind') packagingItemsState.filters.systemType = ''; if (filter === 'status') packagingItemsState.filters.status = 'active'; render(); }
function clearablePackagingFilterChip(label: string, filter: 'search' | 'category' | 'kind' | 'status') { return `<span class="tag-chip selected">${label} <button type="button" data-action="clear-packaging-filter" ${packagingItemSubmitting ? 'disabled' : ''} data-filter="${filter}" aria-label="Убрать фильтр ${label}">×</button></span>`; }
...
function selectStockMovementLot(lotId: number) { if (stockMovementSubmitting) return; stockMovementSubmitToken += 1; stockMovementDetailToken += 1; saveStockMovementFormFromDom(); stockMovementsState.selectedLotId = lotId || null; stockMovementsState.form = { ...emptyStockMovementForm(), ingredient_lot_id: lotId ? String(lotId) : '' }; stockMovementValidation = emptyFormValidationState(); stockMovementsRefreshWarning = ''; stockMovementsMessage = ''; stockMovementsError = ''; render(); if (lotId) loadSelectedStockMovementLot(lotId); }
function loadSelectedStockMovementLot(lotId: number) { const token = ++stockMovementDetailToken; stockMovementsState.detailStatus = 'loading'; stockMovementsState.balance = null; stockMovementsState.movements = []; if (!stockMovementSubmitting) render(); Promise.all([getIngredientLotBalance(lotId), getStockMovementsByLot(lotId)]).then(([balance, movements]) => { if (token !== stockMovementDetailToken || stockMovementsState.selectedLotId !== lotId || stockMovementSubmitting) return; stockMovementsState.balance = balance; stockMovementsState.movements = movements.movements; stockMovementsState.detailStatus = 'ready'; render(); }).catch(() => { if (token !== stockMovementDetailToken || stockMovementsState.selectedLotId !== lotId || stockMovementSubmitting) return; stockMovementsState.detailStatus = 'error'; stockMovementsError = 'Не удалось загрузить остаток или историю выбранной партии.'; render(); }); }
...
function submitPackagingCatalogCategoryForm(event: SubmitEvent) { event.preventDefault(); if (packagingItemSubmitting) return; const name = String(new FormData(event.currentTarget as HTMLFormElement).get('name') ?? '').trim(); if (!name) return; packagingItemsState.catalogCreating = 'category'; packagingItemsError = ''; render(); createPackagingCatalogCategory(name).then(() => reloadPackagingCatalogData()).then(() => { packagingItemsMessage = 'Группа тары создана.'; packagingItemsState.catalogCreating = null; render(); }).catch(() => { packagingItemsState.catalogCreating = null; packagingItemsError = 'Не удалось создать группу тары. Проверьте название и попробуйте еще раз.'; render(); }); }
function submitPackagingCatalogTagForm(event: SubmitEvent) { event.preventDefault(); if (packagingItemSubmitting) return; const name = String(new FormData(event.currentTarget as HTMLFormElement).get('name') ?? '').trim(); if (!name) return; packagingItemsState.catalogCreating = 'tag'; packagingItemsError = ''; render(); createPackagingCatalogTag(name).then(() => reloadPackagingCatalogData()).then(() => { packagingItemsMessage = 'Метка тары создана.'; packagingItemsState.catalogCreating = null; render(); }).catch(() => { packagingItemsState.catalogCreating = null; packagingItemsError = 'Не удалось создать метку тары. Проверьте название и попробуйте еще раз.'; render(); }); }
function updatePackagingDraftCategory(packagingItemId: number, value: string) { if (packagingItemSubmitting) return; if (packagingItemsState.assignmentDraft.itemId !== packagingItemId) return; updateDraftCategory(packagingItemsState.assignmentDraft, value); packagingItemsMessage = ''; render(); }
function updatePackagingDraftTag(packagingItemId: number, tagId: number, checked: boolean) { if (packagingItemSubmitting) return; if (packagingItemsState.assignmentDraft.itemId !== packagingItemId || !tagId) return; updateDraftTag(packagingItemsState.assignmentDraft, tagId, checked); packagingItemsMessage = ''; render(); }
function resetPackagingAssignmentDraft() { if (packagingItemSubmitting) return; const item = packagingItemsState.form.id ? packagingItemsState.items.find((packagingItem) => packagingItem.id === packagingItemsState.form.id) ?? null : null; packagingItemsState.assignmentDraft = resetAssignmentDraft(item); packagingItemsMessage = ''; render(); }
